...
if (description)
{
  ...
  script_name(english:"PHP 5.4.x < 5.4.11 cURL X.509 Certificate Domain Name Matching MiTM Weakness");
...
remote host is prior to 5.4.11.  It is, therefore, potentially 
...
fails to verify that a server hostname matches a domain name in an X.509
certificate's 'Subject Common Name' (CN) or 'SubjectAltName'.

Note that this plugin does not attempt to verify whether the PHP install
has been built with the cURL extention but instead relies only on PHP's
self-reported version number.");
  script_set_attribute(attribute:"see_also", value:"http://www.php.net/ChangeLog-5.php#5.4.11");
...
# nb: we can't tell if PHP was built with cURL support so we'll only run
#     the plugin if report_paranoia == 2.
