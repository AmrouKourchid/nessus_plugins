...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP2 : glibc (EulerOS-SA-2020-1667)");
...
vulnerabilities :
  ...
  - The glibc package contains standard libraries which are
    ...
    will not function.Security Fix(es):Stack-based buffer
    overflow in the getaddrinfo function in
    sysdeps/posix/getaddrinfo.c in the GNU C Library (aka
    ...
    an incomplete fix for CVE-2013-4458.(CVE-2016-3706)In
    ...
    Uncontrolled Recursion, as demonstrated by
    '(\227|)(\\1\\1|t1|\\\2537)+' in
    grep.(CVE-2018-20796)GNU Libc current is affected by:
    Re-mapping current loaded library with malicious ELF
    file. The impact is: In worst case attacker may
    evaluate privileges. The component is: libld. The
    attack vector is: Attacker sends 2 ELF files to victim
    and asks to run ldd on it. ldd execute
    code.(CVE-2019-1010023)A use-after-free vulnerability
    introduced in glibc upstream version 2.14 was found in
    the way the tilde expansion was carried out. Directory
    paths containing an initial tilde followed by a valid
    username were affected by this issue. A local attacker
    could exploit this flaw by creating a specially crafted
    path that, when processed by the glob function, would
    potentially lead to arbitrary code execution. This was
    fixed in version 2.32.(CVE-2020-1752)The GNU C Library
    (aka glibc or libc6) before 2.32 could overflow an
    on-stack buffer during range reduction if an input to
    an 80-bit long double function contains a non-canonical
    bit pattern, a seen when passing a
    0x5d414141414141410000 value to sinl on x86 targets.
    This is related to
...
without introducing additional issues.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2020-1667
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?9a48d1a6");
...
pkgs = ["glibc-2.17-111.h49",
        "glibc-common-2.17-111.h49",
        "glibc-devel-2.17-111.h49",
        "glibc-headers-2.17-111.h49",
        "glibc-static-2.17-111.h49",
        "glibc-utils-2.17-111.h49",
        "nscd-2.17-111.h49"];
