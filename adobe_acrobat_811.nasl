...
described in the vendor advisory.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2007-5020");
  ...
  script_require_keys("installed_sw/Adobe Acrobat", "Host/OS/smb", "SMB/Registry/Enumerated");
...
var os = get_kb_item("Host/OS/smb");
...
var ie = hotfix_check_ie_version();
...
if (version =~ "^(7\.0\.|8\.(0\.|1\.0))")
{
  ...
  else
  {
    ...
    var key = "SOFTWARE\Adobe\Acrobat Acrobat\7.0\FeatureLockDown\cDefaultLaunchURLPerms";
    var key_h = RegOpenKey(handle:hklm, key:key, mode:MAXIMUM_ALLOWED);
    if (!isnull(key_h))
    {
      var value = RegQueryValue(handle:key_h, item:"tSchemePerms");
