...
var key = "SOFTWARE\Legato\Networker";
var item = get_values_from_key(handle:handle, key:key, entries:make_list('Path'));
...
var regversion = get_registry_value(handle:handle, item:key);
...
var reg_serv = get_registry_subkeys(handle:handle, key:key);
var is_server = !isnull(reg_serv);
...
var fileName        = hotfix_append_path(path:path, value:"bin\winworkr.exe");
var fileName_nmm    = hotfix_append_path(path:path, value:"bin\EMC.NetWorker.dll");
var fileName_nmmedi = hotfix_append_path(path:path, value:"bin\nwmedisan.dll");
...
if (item['error'] == HCF_OK)
{
  var version_nmm = join(item['value'], sep:".");
...
if (item['error'] == HCF_OK)
{
  var version_nmmedi = join(item['value'], sep:".");
