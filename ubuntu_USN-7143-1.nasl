...
# extracted from Ubuntu Security Notice USN-7143-1. The text
...
if (description)
{
  ...
  script_xref(name:"USN", value:"7143-1");

  script_name(english:"Ubuntu 20.04 LTS : RabbitMQ Server vulnerabilities (USN-7143-1)");
...
the USN-7143-1 advisory.

    Christian Rellmann discovered that RabbitMQ Server did not properly sanitize user input when adding a new
    user via the management UI. An attacker could possibly use this issue to perform cross site scripting and
    obtain sensitive information. (CVE-2021-32718)

    Fahimhusain Raydurg discovered that RabbitMQ Server did not properly sanitize user input when using the
    federation management plugin. An attacker could possibly use this issue to perform cross site scripting
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://ubuntu.com/security/notices/USN-7143-1");
...
var pkgs = [
    {'osver': '20.04', 'pkgname': 'rabbitmq-server', 'pkgver': '3.8.3-0ubuntu0.2'}
];
