...
"Disable the Telnet service and use SSH instead.");
 ...
 script_set_attribute(attribute:"cvss_score_rationale", value:"Information disclosure vulnerability.");
 ...
 script_dependencies("telnetserver_detect_type_nd_version.nasl", "telnet_starttls.nasl");
...
if (
     (
       (banner =~ "Telnet connection is not allowed" ) ||
       (banner =~ '^Telnet Connection Disabled\r\n$' ) ||
       ("new remote sessions are disallowed by" >< banner)
     )
     && (banner !~ "ogin:") && (banner !~ "word:")
  )
{
  exit(0, "Telnet port "+port+" is open but indicates telnet is not allowed.");
}
if (
  strlen(banner) &&
