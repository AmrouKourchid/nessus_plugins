...
if (description)
{
  ...
  script_name(english:"RHEL 7 : RHV-H security,   update (redhat-virtualization-host) 4.3.13 (Important) (RHSA-2021:0395)");
...
referenced in the RHSA-2021:0395 advisory.
    ...
    redhat-release-virtualization-host. Red Hat Virtualization Hosts (RHVH) are installed using a special
    build of Red Hat Enterprise Linux with only the packages required to host virtual machines. RHVH features
    a Cockpit user interface for monitoring the host's resources and performing administrative tasks.
    ...
    * When performing an upgrade of the Red Hat Virtualization Host using the command `yum update`, the yum
    repository for RHV 4.3 EUS is unreachable
...
number.");
  # https://security.access.redhat.com/data/csaf/v2/advisories/2021/rhsa-2021_0395.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?b2ee20a0");
  ...
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1899378");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1916111");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'redhat-virtualization-host-image-update-4.3.13-20210127.0.el7_9', 'release':'7', 'el_string':'el7_9', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'ovirt-'}
    ]
  }
];
