...
if (description)
{
  ...
  script_name(english:"PHP 5.5.x < 5.5.30 Multiple Vulnerabilities");
...
server is 5.5.x prior to 5.5.30. It is, therefore, affected by the
following vulnerabilities :
  ...
  - An uninitialized pointer flaw exists in the
    ...
    that consists of a single forward slash. An
    unauthenticated, remote attacker can exploit this to
    cause a denial of service condition or to disclose
    sensitive information. (CVE-2015-7804)

Note that Nessus has not tested for these issues but has instead
relied only on the application's self-reported version number.");
  script_set_attribute(attribute:"see_also", value:"http://php.net/ChangeLog-5.php#5.5.30");
