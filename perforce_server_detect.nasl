...
central database to track file versions and user activity.");
  script_set_attribute(attribute:"see_also", value:"https://en.wikipedia.org/wiki/Perforce");
...
)
{
      ...
      while (fmt =~ "%[^%]+%")
      {
        ...
        if (match)
        {
          ...
          else
          {
            ...
            {
              var version;
              var parse_version = pregmatch(string:val, pattern:"[0-9]{4}.[0-9]");
              if (!empty_or_null(parse_version)) version = parse_version[0];
  ...
  {
    var app   = 'Helix Core Server';
    var cpe   = 'cpe:/a:perforce:perforce_server';
    var vendor = 'Helix';
