...
var kb_base = "Antivirus/McAfee_VSEL/";

get_kb_item_or_exit(kb_base+"installed");
var app_name = get_kb_item_or_exit(kb_base+"product_name");

if (app_name !~ "McAfee VirusScan Enterprise for Linux") audit(AUDIT_NOT_INST, app_name);

# Get KB items.
var path = get_kb_item(kb_base+"product_path");
var version = get_kb_item(kb_base+"product_version");
var dat_version = get_kb_item(kb_base+"dat_version");
var engine_version = get_kb_item(kb_base+"engine_version");
var dat_outdated = FALSE;
var engine_outdated = FALSE;
var port = 0;

# Placeholders if needed.
if (isnull(path)) path = UNKNOWN_VER;
...
var latest_engine_version = info["last_engine_version"];
var latest_dat_version = info["datvers"];
