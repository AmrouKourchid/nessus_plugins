...
referenced in the RHSA-2018:3590 advisory.
    ...
    * Previously, a packet was missing the User Datagram Protocol (UDP) payload checksum during a full
    ...
    be received by the peer. (BZ#1635792)

    * Previously, a transform lookup through the xfrm framework could be performed on an already transformed
    destination cache entry (dst_entry). When using User Datagram Protocol (UDP) over IPv6 with a connected
    socket in conjunction with Internet Protocol Security (IPsec) in Encapsulating Security Payload (ESP)
    transport mode. As a consequence, invalid IPv6 fragments transmitted from the host or the kernel
    occasionally terminated unexpectedly due to a socket buffer (SKB) underrun. With this update, the xfrm
    lookup on an already transformed dst_entry is not possible. As a result, using UDP iperf utility over IPv6
    ESP no longer causes invalid IPv6 fragment transmissions or a kernel panic. (BZ#1639586)

Tenable has extracted the preceding description block directly from the Red Hat Enterprise Linux security advisory.

Note that Nessus has not tested for these issues but has instead relied only on the application's self-reported version
number.");
  # https://access.redhat.com/security/data/csaf/v2/advisories/2018/rhsa-2018_3590.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?ceb14b77");
...
"Update the RHEL kernel package based on the guidance in RHSA-2018:3590.");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'kernel-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-debug-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-debug-devel-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-devel-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-headers-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-tools-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-tools-libs-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'kernel-tools-libs-devel-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'perf-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE},
      {'reference':'python-perf-3.10.0-327.76.1.el7', 'sp':'2', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE}
    ]
  }
];
