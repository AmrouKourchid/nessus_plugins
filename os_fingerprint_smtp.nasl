...
var alt_banners, banners, ports_with_banners, banners_to_ports, timestamp_pat, banner, match, alt_banner;
...
foreach port (sort(ports))
{
  ...
  # Add to banners to ports mapping so that we can associate banners with ports.
  if (empty_or_null(banners_to_ports[banner]))
    banners_to_ports[banner] = [];

  append_element(var:banners_to_ports[banner], value:port);
}
if (max_index(banners) == 0) exit(0, "Did not collect any SMTP banners.");


# nb: the SinFP fingerprint is used to adjust the confidence.
var sinfp, sinfp_fingerprint, default_confidence, default_type, kb_base, i, name, smtp_pat, confidence, dev_type;
...
foreach banner (banners)
{
  for (i=0; i<n; i++)
  {
    ...
    if (match)
    {
      ...
      if (!empty_or_null(banners_to_ports[banner]))
      {
        # Just use the first port detected with the banner.
        port = banners_to_ports[banner][0];

        if (port)
        {
          replace_kb_item(name:kb_base+'/Protocol', value:'tcp');
          replace_kb_item(name:kb_base+'/Port', value:port);
        }
      }
