...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP11 : kernel (EulerOS-SA-2024-2207)");
...
the following vulnerabilities :
    ...
    on-existent element. v2: avoid uneeded temporary variable (Stefano)(CVE-2024-26924)
    ...
    address+protection of the anon folio (which is very wrong), or fail on swap
    onswap entries, failing follow_phys() and triggering a WARN_ON_ONCE() in untrack_pfn() and
    track_pfn_copy(), not properly calling free_pfn_range(). In free_pfn_range(), we either wouldn't call
    memtype_free() or would call it with the wrong range, possibly leaking memory. To fix that, let's update
    follow_phys() to refuse returning anon folios, and fallback to using the stored PFN inside vma-vm_pgoff
    for COW mappings if we run into that. We will now properly handle untrack_pfn() with COW mappings, where
    we don't need the cachemode. We'll have to fail fork()-track_pfn_copy() if the first page was replaced
    by an anon folio, though: we'd have to store the cachemode in the VMA to make this work, likely growing
    the VMA size. For now, lets keep it simple and let track_pfn_copy() just fail in that case: it would have
    failed in the past with swap onswap entries already, and it would have done the wrong thing with anon
    folios. Simple reproducer to trigger the WARN_ON_ONCE() in untrack_pfn().(CVE-2024-35877)

    In the Linux kernel, the following vulnerability has been resolved: llc: verify mac len before reading mac
    header LLC reads the mac header with eth_hdr without verifying that the skb has an Ethernet header. Syzbot
    was able to enter llc_rcv on a tun device. Tun can insert packets without mac len and with user
    configurable skb-protocol (passing a tun_pi header when not configuring IFF_NO_PI).(CVE-2023-52843)
    ...
    [1]. Fix by returning if the lists are empty as there is nothing to migrate in this case.(CVE-2024-36006)
    ...
    BUG: KASAN: slab-use-after-free in kfree_skb_list_reason+0x47e/0x4c0 linux
    et/core/skbuff.c:1183 Read of size 8 at addr ffff88804d2a7c80 by task poc/8034.(CVE-2024-36886)
...
number.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2024-2207
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?adae88cb");
