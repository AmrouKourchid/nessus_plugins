...
if (description)
{
  ...
  script_cve_id("CVE-2024-38545");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2024-38545");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: RDMA/hns: Fix UAF for cq async event
    The refcount of CQ is not protected by locks. When CQ asynchronous events and CQ destruction are
    concurrent, CQ may have been released, which will cause UAF. Use the xa_lock() to protect the CQ refcount.
    (CVE-2024-38545)
