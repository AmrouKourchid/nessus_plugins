...
var apps = make_list();
var app, port, weblogic_ports, report, install, version, eol_dates;
...
if (empty_or_null(apps)) audit(AUDIT_NOT_INST, 'Oracle WebLogic Server');
...
eol_dates = make_array(
  ...
  "^12\.1\.3($|[^0-9])",        "2022/01/31",
  "^12\.2\.1\.0($|[^0-9])",     "2017/06/01",
  "^12\.2\.1\.1($|[^0-9])",     "2017/10/01",
  "^12\.2\.1\.2($|[^0-9])",     "2018/08/01",
  "^12\.2\.1\.3($|[^0-9])",     "2023/06/01"
);

var supported_versions = '12.2.1.4 / 14.1.1.0';
var supported = TRUE;

var len = max_index(apps);
var i;
...
for (i = 0; i < len; i++)
{
  ...
  {    
    ...
    {
      if (get_kb_list("www/weblogic/ports"))
      {
        # www
      ...
      if (empty_or_null(version) && get_kb_list("weblogic/ports"))
      {
        # t3 
