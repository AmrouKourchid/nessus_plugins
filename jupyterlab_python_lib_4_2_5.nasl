...
if (description)
{
  ...
  script_name(english:"Jupyterlab Python Library < 3.6.8, < 4.2.5 (CVE-2024-43805)");

  script_set_attribute(attribute:"synopsis", value:
"A Python library installed on the remote host is affected by a vulnerability.");
  script_set_attribute(attribute:"description", value:
"jupyterlab is an extensible environment for interactive and reproducible computing, based on the Jupyter Notebook 
Architecture. This vulnerability depends on user interaction by opening a malicious notebook with Markdown cells, or 
Markdown file using JupyterLab preview feature. A malicious user can access any data that the attacked user has access 
...
"Upgrade to Jupyterlab version 3.6.8, 4.2.5 or later.");
  ...
  script_set_attribute(attribute:"cpe", value:"cpe:/a:jupyter:jupyterlab");
...
else
{
  ...
  os_pkg = ldnix::search_packages([new('ldnix::pkg_target', 'python3-jupyterlab', '(python\\d*-jupyterlab)')]);
  # if the package is found, the host has a version of jupyterlab backported by the OS vendor
...
var constraints = [
  { 'fixed_version' : '3.6.8' },
  { 'min_version' : '4.0', 'fixed_version' : '4.2.5'}
];
