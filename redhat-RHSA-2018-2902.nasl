...
if (description)
{
  ...
  script_name(english:"RHEL 7 : .NET Core on Red Hat Enterprise Linux (RHSA-2018:2902)");
...
the RHSA-2018:2902 advisory.
    ...
    versions are .NET Core 1.1.1 and 1.0.13.
    ...
    * An information disclosure vulnerability exists in .NET Core when authentication information is
    inadvertently exposed in a redirect (CVE-2018-8292)
...
number.");
  # https://security.access.redhat.com/data/csaf/v2/advisories/2018/rhsa-2018_2902.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?99694ead");
  ...
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1636274");
  script_set_attribute(attribute:"solution", value:
"Update the RHEL .NET Core on Red Hat Enterprise Linux package based on the guidance in RHSA-2018:2902.");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'rh-dotnetcore10-dotnetcore-1.0.13-1.el7', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'rh-dotnet10'},
      {'reference':'rh-dotnetcore11-dotnetcore-1.1.10-1.el7', 'cpu':'x86_64', 'release':'7', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'rh-dotnet11'}
    ]
  }
];
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'rh-dotnetcore10-dotnetcore / rh-dotnetcore11-dotnetcore');
}
