...
if (description)
{
  ...
  script_name(english:"Cisco Webex Meetings DLL Injection (cisco-sa-webex-dll-inject-XNmcSGTU)");

  script_set_attribute(attribute:"synopsis", value:
"The remote device is missing a vendor-supplied security patch");
  script_set_attribute(attribute:"description", value:
"According to its self-reported version, Cisco Webex Meetings is affected by an dll injection vulnerability due to 
incorrect handling of directory paths at run time. An authenticated, local attacker can exploit this, by inserting a 
configuration file in a specific path in the system, to execute arbitrary code with the privileges of another user 
account.
...
var app_info = vcf::get_app_info(app:'Cisco Webex Meetings', win_local:TRUE);
