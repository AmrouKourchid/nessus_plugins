...
if (description)
{
  ...
  script_name(english:"Adobe Flash Player <= 30.0.0.134 (APSB18-25)");
...
host is equal or prior to version 30.0.0.134. It is therefore 
affected by multiple vulnerabilities.");
...
foreach variant (variants)
{
  ...
  foreach key (keys(vers))
  {
    ...
    # <= 30.0.0.134
    if (ver_compare(ver:ver,fix:"30.0.0.134",strict:FALSE) <= 0)
    {
      ...
      else if ("Chrome" >< variant)
      {
        ...
        if (variant == "Chrome")
          fix = "Upgrade to a version of Google Chrome running Flash Player 30.0.0.154";
      }
      info += '\n  Path              : ' + file +
              '\n  Installed version : ' + ver;
      if (variant == "Chrome_Pepper")
        info += '\n  Fixed version     : 30.0.0.154 (Chrome PepperFlash)';
      else if (!isnull(fix))
        info += '\n  Fixed version     : '+fix;
      info += '\n';
    }
