...
var cpe, name, login, pass, domain, port, rc, hklm, base_dir, path, prodinfo, sig_path, upd_cfg, ver, autoupdate_status,
prod_subkeys,update_subkeys, name_subkeys, path_subkeys, ver_subkeys, arch, key_list, key, key_h, info, i, subkey, key2,
key2_h, value, data_dir, s, update_path, update_h, key3, key3_h, prod, update_date, contents, error, av_upd, match, tag_open,
subkey_regex, tag_close, xml_file, len, year, month, day, v, sigs_registered, sigs_target, version_registered, trouble, report,
sigs_vendor_yyyymmdd, out_of_date, a, sigs_target_yyyymmdd, sigs_vendor_mmddyyyy, running, services, autoupdate_text, app;
...
dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Arch: ' + arch);
if (arch == 'x64')
{
...
foreach key (key_list)
{
  ...
  for (i=0; i < info[1]; i++)
  {
    ...
    subkey_regex = '^(AVP|KES|PURE|KSVLA)([0-9]+|[0-9\\.]+)?((sp|SP)[0-9]+)?$';

    if (!empty_or_null(subkey) && strlen(subkey) && subkey =~ subkey_regex)
    {
      ## Check if version num associated and assign 2 to use for priority
      if (preg(string:subkey, pattern:"KES([0-9]+|[0-9\\.]+)"))
        subkey_array[key+"\"+subkey]=2;
      else
        subkey_array[key+"\"+subkey]=1;
    }

    foreach var subkey_val (keys(subkey_array))
    {
      if (subkey_array[subkey_val] == 2)
      {
        key2 = subkey_val + "\environment";
...
if (!key2)
{
  ...
  foreach key (key_list)
  {
    ...
    for (i=0; i < info[1]; i++)
    {
      ...
      # example: 10.1.0.0
      subkey = RegEnumKey(handle:key_h, index:i);
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Reading sub key: ' +  obj_rep(subkey) + '\n\n');

      if (!empty_or_null(subkey) && subkey =~ "[0-9.]")
      {
        key2 = key + '\\' + subkey;
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'setting key to ' + key2);
        RegCloseKey(handle:key_h);
        break;
      }
      else
...
if (key2)
{
  dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Connecting to key2: ' +  obj_rep(key2) + '\n\n');
  key2_h = RegOpenKey(handle:hklm, key:key2, mode:MAXIMUM_ALLOWED);
  if (!isnull(key2_h))
  {
    ...
    if (!isnull(value))
    {
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'ProductName determined: ' +  obj_rep(value) + '\n\n');
      name = value[1];
    ...
    if (!isnull(value))
    {
      ...
      {
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'ProdDisplayName determined: ' +  obj_rep(value) + '\n\n');
        name = value[1];
        # get rid of version info in the name
        name = ereg_replace(string:name, pattern:" [0-9.]+", replace:"");
      }
    ...
    if (!isnull(value))
    {
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:' ProductRoot determined:: ' +  obj_rep(value) + '\n\n');
      path = ereg_replace(string:value[1], pattern:"\$", replace:"");
    }

   dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'First attempt to determine Product name and path Finished.\n'
                      + 'Name: ' + name + '\n'
                      + 'Path: ' + path);
    ...
    if (empty_or_null(path))
    {
      if (name == "Kaspersky Security for Windows Server")
      {
        ...
        if (!isnull(value))
        {
          dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'ConnectorPath determined: ' +  obj_rep(value) + '\n\n');
          path = ereg_replace(string:value[1], pattern:"\$", replace:"");
          path = ereg_replace(string:value[1], pattern:"\\ak_conn.dll", replace:"");
          dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:' Product path has not been determined in the first attempt. Set path to : ' +  obj_rep(path) + '\n\n');
        }
      }
    }
    ...
    if (name == "Kaspersky Security for Windows Server")
    {
      ...
      {
        ver = value[1];
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Version determined as ' + ver + ' using regkey ' + key2);
      }
    }
    ...
    if (isnull(ver))
    {
      ...
      if (!isnull(value))
      {
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'ProductDisplayVersion determined:' +  obj_rep(value) + '\n\n');
        ...
        {
          ver = value[1];
          dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'KES v11 determined as ' + ver + ' using regkey ' + key2);
        }
      }
    }
  ...
  dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Connecting to registry key: ' +  obj_rep(update_path) + '\n\n');
  update_h = RegOpenKey(handle:hklm, key:update_path, mode:MAXIMUM_ALLOWED);
  if(isnull(update_h))
  {
    dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'update_h null');
  }
  else
  {
    ...
    if(!isnull(autoupdate_status))
      ...
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'autoupdate_status is null');
  ...
  if (isnull(data_dir) || isnull(base_dir))
  {
    ...
    if (!isnull(key3_h))
    {
      ...
      if (!isnull(value))
      {
        ...
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Signature path: ' + obj_rep(sig_path));
      }
    }
    ...
    if (!isnull(key2_h))
    {
      ...
      if (!isnull(value))
      {
        ...
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Product Info: ' + obj_rep(prodinfo));
      }
    }
    RegCloseKey(handle:key2_h);
  }
...
{
  dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Looking for product using product subkeys');
  ...
  foreach prod (keys(prod_subkeys))
  {
    ...
    if (!isnull(key_h)) {
      ...
      if (!isnull(value))
      {
        dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'ProductVersion determined: ' + obj_rep(value));
        ver = value[1];
      }
...
if (!isnull(upd_cfg) && !isnull(base_dir))
{
  ...
  if (error)
    dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Error occurred while reading antivirus update file ' + upd_cfg + ': ' + error);
  else
    dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Kaspersky antivirus update file ' + upd_cfg + ' content: ' + obj_rep(contents['data']));
  ...
  if (!isnull(av_upd) && isnull(update_date))
  {
    ...
    if (error)
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Error occurred while reading antivirus update file ' + xml_file + ': ' + error);
    else
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Kaspersky antivirus update file ' + xml_file + ' content: ' + obj_rep(contents['data']));
...
else
{
  ...
  if (prodinfo)
  {
    ...
    if (error)
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Error occurred while reading product info file ' + prodinfo + ': ' + error);
    else
      dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Kaspersky product info file ' + prodinfo + ' content: ' + obj_rep(contents['data']));
      ...
      if (!isnull(key_h))
      {
        ...
        if (!isnull(info))
        {
          for (i=0; i < info[1]; i++)
          {
            ...
            # example: 11.0
            subkey = RegEnumKey(handle:key_h, index:i);
            dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Reading sub key: ' + subkey);
            if (!empty_or_null(subkey) && subkey =~ "[0-9.]")
            {
              ...
              dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Parsing Kaspersky update db: ' + value[1]);
              ...
              if (error)
                dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Error occurred while reading Kaspersky update db: ' + value[1] + ': ' + error);
              else
                dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Kaspersky update db ' + value[1] + ' content: ' + obj_rep(contents['data']));
              ...
              dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Found Antivirus Signature (yyyymmddhhmm): ' + obj_rep(match[1]));
  ...
  if (error)
    dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Error occurred while reading signature file ' + sig_path + ': ' + error);
  else
    dbg::detailed_log(lvl:1, src:SCRIPT_NAME, msg:'Kaspersky signature file ' + sig_path + ' content: ' + obj_rep(contents['data']));
