...
if (description)
{
  ...
  script_cve_id("CVE-2024-49977");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2024-49977");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: net: stmmac: Fix zero-division error
    when disabling tc cbs The commit b8c43360f6e4 (net: stmmac: No need to calculate speed divider when
    offload is disabled) allows the port_transmit_rate_kbps to be set to a value of 0, which is then passed
    to the div_s64 function when tc-cbs is disabled. This leads to a zero-division error. When tc-cbs is
    disabled, the idleslope, sendslope, and credit values the credit values are not required to be configured.
    Therefore, adding a return statement after setting the txQ mode to DCB when tc-cbs is disabled would
    prevent a zero-division error. (CVE-2024-49977)
...
"There is no known solution at this time.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2024-49977");
