...
connections.");
 ...
 script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?e979b5cb");
...
"Disable these HTTP methods. Refer to the plugin output for more information.");
 ...
 script_copyright(english:"This script is Copyright (C) 2003-2024 E-Soft Inc.");
...
var file = "/Nessus"+rand() + ".html";	# Does not exist

var cmd1 = http_get(item: file, port:port);
var cmd2 = cmd1;

cmd1 = ereg_replace(pattern:"GET /", string:cmd1, replace:"TRACE /");
cmd2 = ereg_replace(pattern:"GET /", string:cmd2, replace:"TRACK /");

var ua = egrep(pattern:"^User-Agent", string:cmd1, icase:TRUE);

var reply = http_keepalive_send_recv(port:port, data:cmd1, bodyonly:FALSE);

if ( reply == NULL ) exit(0, 'The host is not affected on port ' + port + '.');
  
var r, full_reply;
if ( preg(pattern:"^HTTP/.* 200 ", string:reply) )
{
  r = strstr(reply, '\r\n\r\n');
  if (! r ) r = strstr(reply, '\n\n');
  full_reply = reply;
  ...
  {
    if ( ua && tolower(ua) >!< tolower(reply) ) 
