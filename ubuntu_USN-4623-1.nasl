...
if (description)
{
  ...
  script_name(english:"Ubuntu 16.04 LTS / 18.04 LTS / 20.04 LTS : Pacemaker vulnerability (USN-4623-1)");
...
referenced in the USN-4623-1 advisory.

    Ken Gaillot discovered that Pacemaker incorrectly handled IPC communications permissions. A local attacker
    could possibly use this issue to bypass ACL restrictions and execute arbitrary code as root.
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'libcib-dev / libcib27 / libcib4 / libcrmcluster-dev / etc');
}
