...
if (description)
{
  ...
  script_name(english:"Apache HTTP Server SEoL (1.4.x <= x <= 2.0.x)");

  script_set_attribute(attribute:"synopsis", value:
"An unsupported version of Apache HTTP Server is installed on the remote host.");
  script_set_attribute(attribute:"description", value:
"According to its version, Apache HTTP Server is between 1.4.x and 2.0.x. It is, therefore, no longer maintained by its
vendor or provider.

Lack of support implies that no new security patches for the product will be released by the vendor. As a result, it may
contain security vulnerabilities.");
  script_set_attribute(attribute:"see_also", value:"https://archive.apache.org/dist/httpd/Announcement2.0.txt");
...
"Upgrade to a version of Apache HTTP Server that is currently supported.");
  ...
  script_set_attribute(attribute:"seol_date", value:"2013/07/10");
...
var constraints = [
  { max_branch : '2.0', min_branch : '1.4', seol : 20130710 }
];
