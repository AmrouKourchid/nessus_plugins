...
# extracted from Ubuntu Security Notice USN-5908-1. The text
...
if (description)
{
  ...
  script_xref(name:"USN", value:"5908-1");
  script_xref(name:"IAVA", value:"2023-A-0121-S");

  script_name(english:"Ubuntu 22.04 LTS : Sudo vulnerability (USN-5908-1)");
...
USN-5908-1 advisory.
    ...
    It was discovered that Sudo incorrectly handled the per-command chroot feature. In certain environments
    where Sudo is configured with a rule that contains a CHROOT setting, a local attacker could use this issue
    to cause Sudo to crash, resulting in a denial of service, or possibly escalate privileges.

Tenable has extracted the preceding description block directly from the Ubuntu security advisory.

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://ubuntu.com/security/notices/USN-5908-1");
...
var pkgs = [
    {'osver': '22.04', 'pkgname': 'sudo', 'pkgver': '1.9.9-1ubuntu2.3'},
    {'osver': '22.04', 'pkgname': 'sudo-ldap', 'pkgver': '1.9.9-1ubuntu2.3'}
];
