...
var update_vers = { "^10\.":[ {'ver' : '10.0.30319.1',   'update' : '0',   'name' : 'RTM'},
                ...
                "^18\.":[ {'ver' : '18.170.30525.1',  'update' : '0',    'name' : 'RTW'},
                          ...
                          {'ver' : '18.181.34126.2',  'update' : '1.2',  'name' : 'Update 1.2'}
                        ],
...
foreach path (paths)
{
  ...
  dbg::detailed_log(lvl:3, msg: "[ + ] Enumerated Files : " + '\n' + obj_rep(file_versions));
  ...
  if (ver_compare(ver:version, fix:'17', strict:FALSE) >= 0)
  {
    ...
    var azure_devops_info = [{'Product':product,'Version':version}];
    dbg::detailed_log(lvl:3, msg: "[ + ] Azure DevOps Install Info : " + '\n' + obj_rep(azure_devops_info));
  }
  else
  {
    cpe = "cpe:/a:microsoft:visual_studio_team_foundation_server";
    var team_foundation_info = [{'Product':product,'Version':version}];
    dbg::detailed_log(lvl:3, msg: "[ + ] Team Foundation Server Install Info : " + '\n' + obj_rep(team_foundation_info));
  }

  if (rel && friendly_update && !empty_or_null(update))
  {
    pre_reg_install_info = [{'Release':rel,'Friendly Update':friendly_update,'Update':update,'Target File':exe,'Version':version}]; 
    dbg::detailed_log(lvl:3, msg: "[ + ] Pre-register_install Info - 1 : " + '\n' + obj_rep(pre_reg_install_info));
  ...
  {
    pre_reg_install_info = [{'Target File':exe,'Version':version}]; 
    dbg::detailed_log(lvl:3, msg: "[ + ] Pre-register_install Info - 2 : " + '\n' + obj_rep(pre_reg_install_info));
    register_install(
      app_name : appname,
      vendor   : vendor,
      product  : product,
