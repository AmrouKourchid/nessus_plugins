...
if (description)
{
  ...
  script_name(english:"Tenable Nessus < 10.6.3 (TNS-2023-40)");

  script_set_attribute(attribute:"synopsis", value:
"An instance of Nessus installed on the remote system is affected by a vulnerability.");
  script_set_attribute(attribute:"description", value:
"According to its self-reported version, the Tenable Nessus application running on the remote host is prior to 10.6.3. It
is, therefore, affected by a vulnerability as referenced in the TNS-2023-40 advisory.
  ...
  - An arbitrary file write vulnerability exists where an authenticated, remote attacker with administrator
    ...
    10.6.3 to address these issues. The installation files can be obtained from the Tenable Downloads Portal
    (https://www.tenable.com/downloads/nessus). (CVE-2023-6062)

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://www.tenable.com/security/TNS-2023-40");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Tenable Nessus 10.6.3 or later.");
...
var constraints = [
  { 'min_version' : '10.6.0', 'max_version' : '10.6.2', 'fixed_display' : '10.6.3' }
];
