...
# SUSE update advisory SUSE-SU-2023:0338-1. The text itself
# is copyright (C) SUSE.
##
...
if (description)
{
  ...
  script_xref(name:"SuSE", value:"SUSE-SU-2023:0338-1");

  script_name(english:"SUSE SLES12 Security Update : libapr-util1 (SUSE-SU-2023:0338-1)");
...
number.");
  ...
  # https://lists.suse.com/pipermail/sle-security-updates/2023-February/013715.html
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?18adb41f");
  script_set_attribute(attribute:"solution", value:
"Update the affected libapr-util1 and / or libapr-util1-dbd-sqlite3 packages.");
...
var pkgs = [
    {'reference':'libapr-util1-1.5.3-2.11.1', 'sp':'2', 'cpu':'x86_64', 'release':'SLES12', 'rpm_spec_vers_cmp':TRUE, 'exists_check':['sles-bcl-release-12.2']},
    {'reference':'libapr-util1-dbd-sqlite3-1.5.3-2.11.1', 'sp':'2', 'cpu':'x86_64', 'release':'SLES12', 'rpm_spec_vers_cmp':TRUE, 'exists_check':['sles-bcl-release-12.2']}
];
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'libapr-util1 / libapr-util1-dbd-sqlite3');
}
