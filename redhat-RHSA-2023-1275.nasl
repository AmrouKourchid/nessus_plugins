...
# extracted from Red Hat Security Advisory RHSA-2023:1275. The text
...
if (description)
{
  ...
  script_xref(name:"RHSA", value:"2023:1275");

  script_name(english:"RHEL 8 : Red Hat OpenStack Platform (etcd) (RHSA-2023:1275)");

  script_set_attribute(attribute:"synopsis", value:
"The remote Red Hat host is missing one or more security updates for Red Hat OpenStack Platform (etcd).");
  script_set_attribute(attribute:"description", value:
"The remote Redhat Enterprise Linux 8 host has a package installed that is affected by multiple vulnerabilities as
referenced in the RHSA-2023:1275 advisory.

    etcd is a highly-available key value store for shared configuration.

    The following Important impact security fix(es) are applicable to Red Hat OpenStack Platform 17.0
    (Wallaby), 16.2 (Train), and 16.1 (Train):

    * Improve heuristics preventing CPU/memory abuse by parsing malicious or
    large YAML documents (CVE-2022-3064)

    As a result of being built by golang 1.18.9, the following Moderate impact security fix(es) are applicable
    to Red Hat OpenStack Platform 16.2 and 16.1:
    ...
    * golang: net/http: An attacker can cause excessive memory growth in a Go server accepting HTTP/2 requests
    ...
    As a result of being built by golang 1.18.9, the following Low impact security fix(es) are applicable to
    Red Hat OpenStack Platform 16.2 and 16.1:
...
number.");
  # https://security.access.redhat.com/data/csaf/v2/advisories/2023/rhsa-2023_1275.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?55ab5466");
  ...
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/errata/RHSA-2023:1275");
  script_set_attribute(attribute:"solution", value:
"Update the RHEL Red Hat OpenStack Platform (etcd) package based on the guidance in RHSA-2023:1275.");
  ...
  script_cwe_id(200, 331, 400, 444, 770, 1325);
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'etcd-3.3.23-12.el8ost', 'cpu':'ppc64le', 'release':'8', 'el_string':'el8ost', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'openstack-'},
      {'reference':'etcd-3.3.23-12.el8ost', 'cpu':'x86_64', 'release':'8', 'el_string':'el8ost', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'openstack-'}
    ]
  }
];
