...
# extracted from Mozilla Foundation Security Advisory mfsa2024-33.
...
if (description)
{
  ...
  script_name(english:"Mozilla Firefox < 129.0");

  script_set_attribute(attribute:"synopsis", value:
"A web browser installed on the remote macOS or Mac OS X host is affected by multiple vulnerabilities.");
  script_set_attribute(attribute:"description", value:
"The version of Firefox installed on the remote macOS or Mac OS X host is prior to 129.0. It is, therefore, affected by
multiple vulnerabilities as referenced in the mfsa2024-33 advisory.
  ...
  - A select option could partially obscure security prompts. This could be used by a malicious site to trick
    a user into granting permissions.  This issue only affects Android versions of Firefox. (CVE-2024-7523)
  ...
  - ANGLE failed to initialize parameters which lead to reading from uninitialized memory. This could be
  ...
  - Incorrect garbage collection interaction could have led to a use-after-free. (CVE-2024-7530)

  - Calling <code>PK11Encrypt()</code> in NSS using CKMCHACHA20 and the same buffer for input and output can
    result in plaintext on an Intel Sandy Bridge processor. In Firefox this only affects the QUIC header
    protection feature when the connection is using the ChaCha20-Poly1305 cipher suite. The most likely
    outcome is connection failure, but if the connection persists despite the high packet loss it could be
    possible for a network observer to identify packets as coming from the same source despite a network path
    change. (CVE-2024-7531)

Note that Nessus has not tested for these issues but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://www.mozilla.org/en-US/security/advisories/mfsa2024-33/");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Mozilla Firefox version 129.0 or later.");
...
mozilla_check_version(version:version, path:path, product:'firefox', esr:FALSE, fix:'129.0', severity:SECURITY_HOLE);
