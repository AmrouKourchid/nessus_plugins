...
foreach var package_array ( pkgs ) {
  ...
  if (reference && release) {
    if (exists_check) {
        if (rpm_exists(release:release, rpm:exists_check) && rpm_check(require_epoch_match:TRUE, release:release, sp:sp, cpu:cpu, reference:reference, epoch:epoch, require_epoch_match:TRUE, el_string:el_string, rpm_spec_vers_cmp:rpm_spec_vers_cmp, allowmaj:allowmaj)) flag++;
    } else {
        if (rpm_check(require_epoch_match:TRUE, release:release, sp:sp, cpu:cpu, reference:reference, epoch:epoch, require_epoch_match:TRUE, el_string:el_string, rpm_spec_vers_cmp:rpm_spec_vers_cmp, allowmaj:allowmaj)) flag++;
    }
  }
}
