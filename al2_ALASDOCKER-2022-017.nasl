...
# extracted from Amazon Linux 2 Security Advisory ALASDOCKER-2022-017.
##
...
if (description)
{
  ...
  script_name(english:"Amazon Linux 2 : docker (ALASDOCKER-2022-017)");
...
vulnerabilities as referenced in the ALAS2DOCKER-2022-017 advisory.

    A file permissions vulnerability was found in Moby (Docker Engine). Copying files by using `docker cp`
    into a specially-crafted container can result in Unix file permission changes for existing files in the
    host's filesystem, which might lead to permissions escalation and allow an attacker access to restricted
    data. (CVE-2021-41089)
