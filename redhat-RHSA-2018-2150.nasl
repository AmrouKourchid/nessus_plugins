...
referenced in the RHSA-2018:2150 advisory.
    ...
    * Restore module_utils.basic.BOOLEANS variable for backwards compatibility with the module API in older
    ansible releases.
    ...
    * influxdb_query - fixed the use of the common return 'results' caused an unexpected fault. The return is
    renamed to 'query_results'

    * junos_config - fix confirm commit timeout issue (https://github.com/ansible/ansible/pull/41527)

    * lineinfile - fix insertbefore when used with BOF to not insert duplicate lines
    ...
    * win_iis_webapppool - redirect some module output to null so Ansible can read the output JSON
    https://github.com/ansible/ansible/issues/40874

    * win_updates - Fixed issue where running win_updates on async fails without any error

    * winrm - ensure pexpect is set to not echo the input on a failure and have a manual sanity check
    afterwards https://github.com/ansible/ansible/issues/41865

Tenable has extracted the preceding description block directly from the Red Hat Enterprise Linux security advisory.

Note that Nessus has not tested for these issues but has instead relied only on the application's self-reported version
number.");
  # https://access.redhat.com/security/data/csaf/v2/advisories/2018/rhsa-2018_2150.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?c744196c");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/errata/RHSA-2018:2150");
...
"Update the RHEL ansible package based on the guidance in RHSA-2018:2150.");
...
var constraints = [
  {
    'repo_relative_urls': [
      'content/dist/rhel/power-le/7/7Server/ppc64le/ansible/2.5/debug',
      'content/dist/rhel/power-le/7/7Server/ppc64le/ansible/2.5/os',
      'content/dist/rhel/power-le/7/7Server/ppc64le/ansible/2.5/source/SRPMS',
      'content/dist/rhel/server/7/7Server/x86_64/ansible/2.5/debug',
      'content/dist/rhel/server/7/7Server/x86_64/ansible/2.5/os',
      'content/dist/rhel/server/7/7Server/x86_64/ansible/2.5/source/SRPMS'
    ],
    'pkgs': [
      {'reference':'ansible-2.5.6-1.el7ae', 'release':'7', 'el_string':'el7ae', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'ansible-2.5'},
      {'reference':'ansible-doc-2.5.6-1.el7ae', 'release':'7', 'el_string':'el7ae', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'ansible-2.5'}
    ]
  }
];
