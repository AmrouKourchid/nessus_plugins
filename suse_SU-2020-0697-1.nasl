...
# extracted from SUSE update advisory SUSE-SU-2020:0697-1.
...
if (description)
{
  ...
  script_name(english:"SUSE SLES15 Security Update : cni, cni-plugins, conmon, fuse-overlayfs, podman (SUSE-SU-2020:0697-1)");
...
CVE-2019-18466: Fixed a bug where podman cp would improperly copy
files on the host when copying a symlink in the container that
included a glob operator (#3829 bsc#1155217)

The name of the cni-bridge in the default config changed from 'cni0'
to 'podman-cni0' with podman-1.6.0. Add a %trigger to rename the
bridge in the system to the new default if it exists. The trigger is
only excuted when updating podman-cni-config from something older than
1.6.0. This is mainly needed for SLE where we're updating from 1.4.4
to 1.8.0 (bsc#1160460).

Update podman to v1.8.0 (bsc#1160460): Features
  ...
  - Fixed a bug where rootless podman run and podman create
    ...
    --userns=keep-id option could change permissions on
  ...
  - Fixed a bug where named volumes created with containers
    ...
    with their containers if the

    --rm flag was given, even if they were given names
  ...
  - Fixed a bug where the podman build command would
    ...
    -f option and build the wrong Containerfile
  ...
  - Fixed a bug where setting the --memory-swap flag to
    ...
    unlimited) was not supported Misc
  ...
  - Updated vendored containers/image to v5.2.0 Add
    apparmor-abstractions as required runtime dependency to
...
Update podman to v1.7.0 Features
  ...
  - The podman info command now displays information on
    registry mirrors (#4553) Bugfixes
  ...
  - Fixed a bug where containers started with --rm would not
    ...
    session was running inside the container (#4666) Misc
...
Update podman to v1.6.4 Remove winsz FIFO on container restart to
allow use with Conmon 2.03 and higher
...
Update podman to v1.6.2 Features
  ...
  - The --systemd flag to podman run and podman create now
    ...
    the the container entrypoint is systemd Bugfixes
  ...
  - Fixed a bug where containers restored via podman
    ...
    --import would retain the CGroup path of the original
    container, even if their container ID changed; thus,
    multiple containers created from the same checkpoint
    would all share the same CGroup Misc
...
Update podman to v1.6.1 Features
  ...
  - The output of podman volume inspect has been more
    closely matched to docker volume inspect Add
    katacontainers as a recommended package, and include it
    as an additional OCI runtime in the configuration.

Update podman to v1.5.1 Features
  ...
  - Podman now properly sets a user agent while contacting
    registries (#3788) Add zsh completion for podman
    commands

Update podman to v1.5.0 Features
...
fuse-overlayfs was updated to v0.7.6 (bsc#1160460) do not look in
lower layers for the ino if there is no origin xattr set

attempt to use the file path if the operation on the fd fails with
ENXIO
...
ignore O_DIRECT. It causes issues with libfuse not using an aligned
buffer, causing write(2) to fail with EINVAL.

on copyup, do not copy the opaque xattr.

fix a wrong lookup for whiteout files, that could happen on a double
unlink.
...
cni was updated to 0.7.1: Set correct CNI version for 99-loopback.conf

Update to version 0.7.1 (bsc#1160460): Library changes :
  ...
  + delegate : allow delegation funcs override CNI_COMMAND
    env automatically in heritance Documentation &
    Convention changes :
...
Update to version 0.7.0: Spec changes :
  ...
  + SPEC.md: clarify meaning of 'routes' Library changes :
...
Update to version 0.8.4 (bsc#1160460): add support for mips64le
...
Update to version 0.8.3: Enhancements :
...
Update plugins to v0.8.2 New features :
  ...
  - pkg/veth: Make host-side veth name configurable Bug
    fixes :
...
Updated plugins to v0.8.1: Bugs :
  ...
  - bandwidth: get bandwidth interface in host ns through
    container interface Improvements :

  - host-device: add pciBusID property

Updated plugins to v0.8.0: New plugins :
  ...
  - win-bridge, win-overlay: Windows plugins Plugin features
    / changelog :
  ...
  - adds 'ip link' command related feature into tuning Bug
    fixes & minor changes
...
from version v0.7.5: This release takes a minor change to the portmap
plugin :

  - Portmap: append, rather than prepend, entry rules

This fixes a potential issue where firewall rules may be bypassed by
port mapping
...
without introducing additional issues.");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.suse.com/show_bug.cgi?id=1155217");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.suse.com/show_bug.cgi?id=1160460");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.suse.com/show_bug.cgi?id=1164390");
  script_set_attribute(attribute:"see_also", value:"https://github.com/containers/libpod/issues/4844");
  script_set_attribute(attribute:"see_also", value:"https://www.suse.com/security/cve/CVE-2019-18466/");
  # https://www.suse.com/support/update/announcement/2020/suse-su-20200697-1/
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?8e52eb3d");
...
patch SUSE-SLE-Module-Public-Cloud-15-SP1-2020-697=1

SUSE Linux Enterprise Module for Containers 15-SP1:zypper in -t patch
SUSE-SLE-Module-Containers-15-SP1-2020-697=1");
...
flag = 0;
if (rpm_check(release:"SLES15", sp:"1", reference:"cni-0.7.1-3.3.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"cni-plugins-0.8.4-3.3.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"conmon-2.0.10-3.3.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"conmon-debuginfo-2.0.10-3.3.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"fuse-overlayfs-0.7.6-3.6.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"fuse-overlayfs-debuginfo-0.7.6-3.6.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"fuse-overlayfs-debugsource-0.7.6-3.6.1")) flag++;
if (rpm_check(release:"SLES15", sp:"1", reference:"podman-1.8.0-4.14.1")) flag++;
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, "cni / cni-plugins / conmon / fuse-overlayfs / podman");
}
