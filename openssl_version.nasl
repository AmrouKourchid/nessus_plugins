...
var app_name = "OpenSSL";
var pattern = "OpenSSL\/(\d+(?:\.\d+)*(-beta\d+|-pre\d+|[a-z]*))";

# Get the pristine banner first.
var pristine_banner = http_server_header(port:port);
var match;
...
if (!preg(string:pristine_banner, pattern:pattern, icase:TRUE))
{
  ...
  var lines = split(res, keep:0);
  if (lines[0] !~ "^HTTP/")
  {
    if ('<title>Bad request!</title>' >< res && '<h2>Error 400</h2>' >< res)
    {
      var i1 = stridx(res, '<span>');
      if (i1 >= 0)
      {
        var i2 = stridx(res, '</span>', i1);
        if (i2 >= 0)
        {
...
var pristine_version = match[1];


# Check if the banner is backported.
var backported_version = NULL;
backported = FALSE;

var backported_banner = get_backport_banner(banner:pristine_banner);
...
report_installs(app_name: app_name, port:port, report_override:report);
