...
if (description)
{
  ...
  script_name(english:"Apache HTTP Server SEoL (<= 1.3.x)");

  script_set_attribute(attribute:"synopsis", value:
"An unsupported version of Apache HTTP Server is installed on the remote host.");
  script_set_attribute(attribute:"description", value:
"According to its version, Apache HTTP Server is less than or equal to 1.3.x. It is, therefore, no longer maintained by
...
"Upgrade to a version of Apache HTTP Server that is currently supported.");
  ...
  script_set_attribute(attribute:"seol_date", value:"2010/02/02");
