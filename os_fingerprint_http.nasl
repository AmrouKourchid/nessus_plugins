...
var os_list, confidence, os_str, os, http_port, ports, brokenports, broken_reason,
banner, v, svr, version, line, match, a, type, slash, reporter_os,
lower_banner, value, match2, firmware;
...
if (!isnull(os_list))
{
  ...
  foreach os (split(os_list, keep:FALSE))
  {
    ...
    os = "Linux Kernel 3.10 on " + os;
    else if ( "Rocky Linux release 8.5" >< os )
    os = "Linux Kernel 4.18 on " + os;
    else if ( "AlmaLinux release 8.4" >< os )
    os = "Linux Kernel 4.18 on " + os;
    else confidence -= 20;
  ...
  http_port = get_one_kb_item('Host/Linux/Distribution/Port');
  if (!empty_or_null(http_port))
  {
    replace_kb_item(name:'Host/OS/HTTP/Protocol', value:'tcp');
    replace_kb_item(name:'Host/OS/HTTP/Port', value:http_port);
  }
...
foreach var p ( keys(brokenports) )
{
...
foreach var port ( ports )
{
 ...
 else if (svr == "Server: Apache" && "(rocky)" >< banner)
 {
   set_kb_item(name:"Host/OS/HTTP", value:"Rocky Linux");
   set_kb_item(name:"Host/OS/HTTP/Confidence", value:70);
   set_kb_item(name:"Host/OS/HTTP/Type", value:"general-purpose");
   exit(0);
 }
 else if (svr == "Server: Apache" && "(AlmaLinux)" >< banner)
 {
   set_kb_item(name:"Host/OS/HTTP", value:"AlmaLinux");
   set_kb_item(name:"Host/OS/HTTP/Confidence", value:70);
   set_kb_item(name:"Host/OS/HTTP/Type", value:"general-purpose");
   exit(0);
 }
 else if (
   'alt="GigaVUE-OS">' >< slash &&
   '<title>GigaVUE-OS' >< slash &&
   'alt="Gigamon">' >< slash
 )
 {
   set_kb_item(name:"Host/OS/HTTP", value:"Gigmon GigaVUE");
   set_kb_item(name:"Host/OS/HTTP/Confidence", value:70);
   set_kb_item(name:"Host/OS/HTTP/Type", value:"embedded");
   exit(0);
 }
