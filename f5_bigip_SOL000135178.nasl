...
"F5 lists a workaround with instructions listed at https://my.f5.com/manage/s/article/K000135178 that can be achieved using
the following steps:

  1. Do not configure client certificate  or server certificate authentication for the related SSL profiles.
  2. Do not configure httpd for client-side certificate authentication in the server
...
Tenable is not responsible for any negative effects that may occur from enacting this workaround.");
  script_set_attribute(attribute:"workaround_publication_date", value:"2023/03/21");
