...
web applications, is running on the remote host.");
  script_set_attribute(attribute:"see_also", value:"https://www.ibm.com/cloud/websphere-application-platform");
  ...
  script_require_ports("Services/www", 8880, 8881, 9100, 9043);
...
if (!isnull(server_name))
{
  ...
  item = pregmatch(pattern:pattern, string:server_name);
  if (!isnull(item))
  {
    source = server_name;
    version = item[1];
...
if (version !~ "^[0-9]+(\.[0-9]+){2,}")
{
  ...
  spad_log(message:'GIOP service returned data that should contain version:\n' + hexdump(ddata:data));

  pattern = strcat(
    "IBM WebSphere Application Server[ -]*(Express|Network Deployment|ND)?",
    "[^\x20-\x7E]+", # Non-standard characters
    "([0-9]+\.[0-9.]+)"
  );
  ...
  if (!isnull(item))
  {
    source = 'GIOP service';
    version = item[2];
    # If the product name conatins either Express|Network Deployment|ND
    # set the name found to be used for extra['sw_edition']
    # If name does not contain either Express|Network Deployment|ND
    # Return the basic sw_edition of 'IBM WebSphere Application Server'
    if (!isnull(item[1]))
    {
      extra['sw_edition'] = item[1];
    }
    else
    {
      extra['sw_edition'] = item[0];
    }
  }
}
