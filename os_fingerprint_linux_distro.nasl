...
#Must precede SuSE because EMC RSA is SuSE
if ( ! os )
{
  os = get_kb_item("Host/EMC/AM/Version");
  if ( os )
  {
    os = "EMC RSA Authentication Manager Appliance " + os;
  }
}
...
if ( ! os )
{
  ...
  if ( os )
  {
    v = pregmatch(string: os, pattern: "(SUSE|SLE[SD]|TDC)([0-9.]+)");
    if (! isnull(v))
    {
      if (v[1] == 'SUSE')
        os = 'openSUSE ' + v[2];
      else if(v[1] == 'TDC')
        os = 'Teradata SLES ' + v[2];
      else os = 'SuSE' + v[2];
    }
    else if ("SUSE" >< os)
    {
...
if ( ! os )
{
  os = get_kb_item("Host/UOS/release");
  if ( os ) os = "Unity Operating System " + os;
}
...
if (!os)
{
  os = get_kb_item("Host/TencentOS/Product");
  if (os) os = "Tencent Linux";
}

if(!os)
{
  os = get_kb_item("Host/PhotonOS/release");
}

if(!os)
{
  os = get_kb_item("Host/AzureLinux/release");
}

if(!os)
{
  os = get_kb_item("Host/OracleVM/release");
}

if ( ! os )
{
  os = get_kb_item("Host/Alibaba/release");
}

if(!isnull(get_kb_item("Host/Avaya/Product")))
  os = get_kb_item("Host/Avaya/Product");
