...
{
  item = "/SSI/info_configuration.htm";
  res = http_send_recv3(method:"GET", port:port, item:item, exit_on_fail:TRUE);
  if ("LaserJet" >!< res[2])
  {
    item = "/info_configuration.html";
    res = http_send_recv3(method:"GET", port:port, item:item, exit_on_fail:TRUE);
  }
    # - Grab the product model
...
# Report unique identifier via structured data
if (data["serial"])
{
  var snc, id_data, msg;
  ...
  id_data = { 
    'identifier_source' : 'hpLaserJetSerialNumber', 
    'identifier_value' : data["serial"],
    'type' : 'serialnumber'
  };
