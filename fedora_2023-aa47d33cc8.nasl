...
# extracted from Fedora Security Advisory FEDORA-2023-aa47d33cc8
#
...
if (description)
{
  ...
  script_xref(name:"FEDORA", value:"2023-aa47d33cc8");

  script_name(english:"Fedora 38 : openbgpd (2023-aa47d33cc8)");
...
FEDORA-2023-aa47d33cc8 advisory.

    # OpenBGPD 7.8

      * Improved performance by optimising the output filters
      * Add Autonomous System Provider Authorization (ASPA) validaton based on draft-ietf-sidrops-aspa-
    verification-12
      ...
      * Improve open policy (RFC 9234) support and enable the capability automatically if a role is specified
    for the peer
      * Introduce a per neighbor `role` configuration option to specify the session role used by ASPA
    verification and the open policy capability. The `announce policy` statement was simplified at the same
    time.
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://bodhi.fedoraproject.org/updates/FEDORA-2023-aa47d33cc8");
...
"Update the affected openbgpd package.");
  ...
  script_set_attribute(attribute:"cpe", value:"p-cpe:/a:fedoraproject:fedora:openbgpd");
...
var pkgs = [
    {'reference':'openbgpd-7.8-1.fc38', 'release':'FC38', 'rpm_spec_vers_cmp':TRUE}
];
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'openbgpd');
}
