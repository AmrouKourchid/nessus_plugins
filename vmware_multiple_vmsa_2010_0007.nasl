...
if (description)
{
  ...
  script_xref(name:"IAVA", value:"2010-A-0066-S");
...
the remote host has one or more of the following vulnerabilities :
  ...
  script_set_attribute(attribute:"see_also", value:"https://www.nessus.org/u?8db51821");
  ...
  # http://web.archive.org/web/20160305233340/http://dsecrg.com/pages/vul/show.php?id=153
  script_set_attribute(attribute:"see_also", value:"https://www.nessus.org/u?82037430");
...
VMware advisory for more information.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2009-3732");
  ...
  script_require_ports("VMware/Player/Version", "Host/VMware Workstation/Version", 139, 445);
...
var vmrc_installed = FALSE;
var decoder_installed = FALSE;
foreach var name (list)
{
...
else if (decoder_installed)
{
  ...
  {
    vuln = TRUE;
    var hf_report = split(hotfix_get_report(), sep:'\n', keep:FALSE);
...
if (vmrc_installed)
{
  ...
  if (fh)
  {
    var ver = GetProductVersion(handle:fh);
    CloseFile(handle:fh);
    NetUseDel();
  }
  else
