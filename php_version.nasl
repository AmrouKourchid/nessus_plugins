...
var app, cpe, port, source, version, installs, banner, pat, matches, item, vers, dir, report, path, extra, sources_str, sources;
...
if (!isnull(banner))
{
  ...
  matches = pgrep(icase:TRUE, string:banner, pattern:pat);
  if (matches)
  {
    foreach var line (split(matches, keep:FALSE))
    {
      backported = FALSE;
      item = pregmatch(icase:TRUE, pattern:pat, string:line);
      if (!isnull(item))
      {
        source = line;
        version = item[2];
...
if (!isnull(vers))
{
  foreach var ver (list_uniq(keys(vers)))
  {
    backported = FALSE;
