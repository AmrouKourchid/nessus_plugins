...
if (description)
{
  ...
  script_name(english:"Atlassian Confluence 1.0.1 < 7.19.23 / 7.20.x < 8.5.9 / 8.6.x < 8.9.1 (CONFSERVER-95943)");
...
the CONFSERVER-95943 advisory.

  - Applications that use UriComponentsBuilder in Spring Framework to parse an externally provided URL (e.g.
    through a query parameter) AND perform validation checks on the host of the parsed URL may be vulnerable
    to a open redirect https://cwe.mitre.org/data/definitions/601.html attack or to a SSRF attack if the URL
    is used after passing validation checks. This is the same as CVE-2024-22243
    https://spring.io/security/cve-2024-22243 , but with different input. (CVE-2024-22259)

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://jira.atlassian.com/browse/CONFSERVER-95943");
...
"Upgrade to Atlassian Confluence version 7.19.23, 8.5.9, 8.9.1 or later.");
  ...
  script_set_attribute(attribute:"vuln_publication_date", value:"2024/03/16");
