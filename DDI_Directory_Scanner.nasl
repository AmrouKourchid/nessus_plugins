...
directory or not." );
	...
	script_set_attribute(attribute:"see_also", value:"http://projects.webappsec.org/w/page/13246953/Predictable%20Resource%20Location");
...
"2002/06/26");
 script_set_attribute(attribute:"plugin_modification_date", value:"2024/06/07");
	...
	script_copyright(english:"This script is Copyright (C) 2002-2024 Digital Defense Inc.");
	...
	script_timeout(43200);	# timeout is managed by the script
	exit(0);
}
...
var report_tag_prefix = "DDI_Dir_Scanner_";
var global_init_time = unixtime();
report_xml_tag(tag:strcat(report_tag_prefix, "Global_Init"), value:string(global_init_time));
...
dirs[i++] = "pma/doc";
dirs[i++] = "pma/doc/html";
dirs[i++] = "pma/doc/html/_sources";
dirs[i++] = "pma/doc/html/_static";
dirs[i++] = "pma/js/dist/server";
dirs[i++] = "pma/js/dist/setup";
dirs[i++] = "pma/js/dist/table";
dirs[i++] = "pma/js/dist/jqplot";
dirs[i++] = "pma/js/dist/designer";
dirs[i++] = "pma/js/dist/database";
dirs[i++] = "pma/js/dist/codemirror";
dirs[i++] = "pma/js/dist";
dirs[i++] = "pmos";
dirs[i++] = "pmwiki";
dirs[i++] = "podcast";
...
var init_time = unixtime();
report_xml_tag(tag:strcat(report_tag_prefix, "Port_", string(port), "_Init"), value:string(init_time));
...
for (dcp=0; dirs[dcp] ; dcp=dcp+1)
{
    ...
    start_pass = unixtime();
    report_xml_tag(tag:strcat(report_tag_prefix, "Port_", string(port), "_Pass_Start"), value:string(start_pass));
    if(debug)display(":: Starting the directory scan...\n");
    for(i = 0; cdirs[i]; i ++)
    {
        current_time = unixtime();
        if (current_time - start_pass >= timeout_per_port)
        {
        debug_print("Stopping on port ", port, " after ", timeout_per_port, " seconds\n");
        report_xml_tag(tag:strcat(report_tag_prefix, "Port_", string(port), "_Pass_Timeout"), value:string(current_time));
        break;
        }
...
report_xml_tag(tag:strcat(report_tag_prefix, "Port_", string(port), "_Duration"), value:string(unixtime() - init_time));
report_xml_tag(tag:strcat(report_tag_prefix, "Global_Duration"), value:string(unixtime() - global_init_time));
