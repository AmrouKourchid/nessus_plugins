...
if (description)
{
  ...
  script_bugtraq_id(44716);

  script_name(english:"jRSS Widget Plugin for WordPress proxy.php 'url' Parameter Arbitrary File Access");

  script_set_attribute(attribute:"synopsis", value:
"The remote web server hosts a PHP script affected by an information
disclosure vulnerability.");
  script_set_attribute(attribute:"description", value:
"The version of the jRSS Widget plugin for WordPress installed on the
remote host does not sanitize input to the 'url' parameter of the
'proxy.php' script before using it to return the contents of a file.
...
privileges under which the web server operates.");
  # https://web.archive.org/web/20110809130917/http://wordpress.org/extend/plugins/jrss-widget/changelog/
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?ab246dc9");
...
if (!installed)
{
  ...
  checks[path + "jrss-widget/js/jfeed/src/jfeed.js"][0] =
    make_list('jQuery\\.getFeed');
...
foreach file (files)
{
  ...
  if (file[0] == '/') traversal = "";
  else traversal = crap(data:"../", length:3*3);

  url = dir + '/wp-content/plugins/jrss-widget/proxy.php?' +
    'url=' + traversal + file + '%00';
  ...
  if (headers['content-type'] && 'application/xml' >< headers['content-type'])
  {
    installs++;
  }
