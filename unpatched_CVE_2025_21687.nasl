...
if (description)
{
  ...
  script_cve_id("CVE-2025-21687");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2025-21687");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: vfio/platform: check the bounds of
    read/write syscalls count and offset are passed from user space and not checked, only offset is capped to
    40 bits, which can be used to read/write out of bounds of the device. (CVE-2025-21687)
