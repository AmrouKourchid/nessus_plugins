...
# extracted from Mozilla Foundation Security Advisory mfsa2025-04.
...
if (description)
{
  ...
  script_name(english:"Mozilla Thunderbird < 134.0");

  script_set_attribute(attribute:"synopsis", value:
"A mail client installed on the remote macOS or Mac OS X host is affected by multiple vulnerabilities.");
  script_set_attribute(attribute:"description", value:
"The version of Thunderbird installed on the remote macOS or Mac OS X host is prior to 134.0. It is, therefore, affected
by multiple vulnerabilities as referenced in the mfsa2025-04 advisory.

  - Memory safety bugs present in Firefox 133 and Thunderbird 133. Some of these bugs showed evidence of
    memory corruption and we presume that with enough effort some of these could have been exploited to run
    arbitrary code. (CVE-2025-0247)
  ...
  - The Matrix specification demands homeservers to perform validation of the server-name and media-id
    components of MXC URIs with the intent to prevent path traversal. However, it is not mentioned that a
    similar check must also be performed on the client to prevent client-side path traversal. matrix-js-sdk
    fails to perform this validation. (CVE-2024-50336)
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://www.mozilla.org/en-US/security/advisories/mfsa2025-04/");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Mozilla Thunderbird version 134.0 or later.");
...
mozilla_check_version(version:version, path:path, product:'thunderbird', esr:FALSE, fix:'134.0', severity:SECURITY_HOLE);
