...
# extracted from Ubuntu Security Notice USN-2979-2. The text 
...
if (description)
{
  ...
  script_xref(name:"USN", value:"2979-2");

  script_name(english:"Ubuntu 14.04 LTS : Linux kernel (Xenial HWE) vulnerabilities (USN-2979-2)");
...
the USN-2979-2 advisory.

    USN-2979-1 fixed vulnerabilities in the Linux kernel for Ubuntu 16.04 LTS. This update provides the
    corresponding updates for the Linux Hardware Enablement (HWE) kernel from Ubuntu 16.04 LTS for Ubuntu
    14.04 LTS.

    David Matlack discovered that the Kernel-based Virtual Machine (KVM) implementation in the Linux kernel
    did not properly restrict variable Memory Type Range Registers (MTRR) in KVM guests. A privileged user in
    a guest VM could use this to cause a denial of service (system crash) in the host, expose sensitive
    information from the host, or possibly gain administrative privileges in the host. (CVE-2016-3713)
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://ubuntu.com/security/notices/USN-2979-2");
...
{
  audit(AUDIT_PATCH_INSTALLED, 'Kernel package for USN-2979-2');
}

if (get_one_kb_item('Host/ksplice/kernel-cves'))
{
  var cve_list = make_list('CVE-2016-0758', 'CVE-2016-3713');
  if (ksplice_cves_check(cve_list))
  {
    audit(AUDIT_PATCH_INSTALLED, 'KSplice hotfix for USN-2979-2');
  }
  else
  {
