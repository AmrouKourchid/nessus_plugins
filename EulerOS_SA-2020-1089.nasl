...
if (description)
{
  ...
  script_name(english:"EulerOS Virtualization for ARM 64 3.0.5.0 : libssh2 (EulerOS-SA-2020-1089)");
...
affected by the following vulnerability :
  ...
  - In libssh2 v1.9.0 and earlier versions, the
    ...
    overflow in a bounds check, enabling an attacker to
    specify an arbitrary (out-of-bounds) offset for a
    subsequent memory read. A crafted SSH server may be
    able to disclose sensitive information or cause a
    denial of service condition on the client system when a
    user connects to the server.(CVE-2019-17498)

Note that Tenable Network Security has extracted the preceding
description block directly from the EulerOS security advisory. Tenable
has attempted to automatically clean and format it as much as possible
without introducing additional issues.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2020-1089
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?223f77a1");
...
pkgs = ["libssh2-1.8.0-8.h10.eulerosv2r8"];
