...
# extracted from openSUSE Security Update openSUSE-2018-1547.
#
...
if (description)
{
  ...
  script_name(english:"openSUSE Security Update : phpMyAdmin (openSUSE-2018-1547)");
...
Security issues addressed in the 4.8.4 release (bsc#1119245) :
  ...
  - CVE-2018-19968: Local file inclusion through
...
improvements :
  ...
  - Ensure that database names with a dot ('.') are handled
    properly when DisableIS is true

  - Fix for message 'Error while copying database
    (pma__column_info)'

  - Move operation causes 'SELECT * FROM `undefined`' error

  - When logging with $cfg['AuthLog'] to syslog, successful
    login messages were not logged when
    $cfg['AuthLogSuccess'] was true

  - Multiple errors and regressions with Designer");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.opensuse.org/show_bug.cgi?id=1119245");
...
if ( rpm_check(release:"SUSE15.0", reference:"phpMyAdmin-4.8.4-lp150.2.12.1") ) flag++;
if ( rpm_check(release:"SUSE42.3", reference:"phpMyAdmin-4.8.4-24.1") ) flag++;
