...
if (description)
{
  ...
  script_name(english:"Microsoft Office Unsupported Channel Version Detection");
...
and Microsoft Office Retail on the remote Windows host is no longer supported. Refer to links in
...
"Upgrade to a Channel version of Microsoft Office that is currently
supported.");
...
var kb_list = make_nested_array();
var report  = NULL;

var version_mappings = make_array(
  "16.0", "2016"
);
...
{
  var office_suite_supported_vers_kb = "SMB/Office/" + ver + "/SupportedVersions";
  var office_suite_supported_vers = get_kb_item(office_suite_supported_vers_kb);
...
foreach kb (keys(kb_list))
{
  ...
  var supported_versions = kb_list[kb]; #
  var kb_base = kb - "SupportedVersions";

  var channel = get_kb_item(kb_base + "Channel"); # Current / First Release for Deferred / Deferred 
  if (isnull(channel))
  {
  ...
  var channel_version = get_kb_item(kb_base + "ChannelVersion");
  ...
  var channel_build = get_kb_item(kb_base + "ChannelBuild");
  ...
  var product_version = version_mappings[office_ver];

  # Check if this Office 365 is present
  # 2016 Office and Office 365 numbers have the same versioning
  if (get_kb_item_or_exit("SMB/Office/365"))
  {
    product_version = "365";
  }
  ...
  var supported_versions_list = sort(split(supported_versions, sep:" / ", keep:FALSE));
  var lowest_supported_version = supported_versions_list[0];

  # CS-63561 / RES-169364
  # 'unknown' version casted as integer -> the same as a variable being assigned 0
  # choose the lowest supported version, to prevent UNKNOWN_VER being assigned to channel_version
  if (int(channel_version) == 0 || channel_version == UNKNOWN_VER)
  {
    channel_version = int(lowest_supported_version);
  }
  
  if (int(channel_version) >= int(lowest_supported_version)) continue;

  var supported_version_str = "Supported version";
  if (max_index(supported_versions_list) > 1) supported_version_str += "s";

  var items = make_array();
  ...
  var order = make_list("Installed product", "Channel", "Channel version", "Channel build", supported_version_str);
  report = report_items_str(report_items:items, ordered_fields:order);

  var cpe_base = str_replace(string:tolower(product), find:" ", replace:":");
  var version = product_version + " " + channel + " " + channel_version;
