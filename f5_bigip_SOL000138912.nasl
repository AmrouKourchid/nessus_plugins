...
# extracted from F5 Networks BIG-IP Solution K000138912.
#
# @NOAGENT@
##
...
if (description)
{
  ...
  script_cve_id("CVE-2024-28889");

  script_name(english:"F5 Networks BIG-IP : BIG-IP SSL vulnerability (K000138912)");
...
therefore, affected by a vulnerability as referenced in the K000138912 advisory.

  - When an SSL profile with alert timeout is configured with a non-default value on a virtual server,
    undisclosed traffic along with conditions beyond the attacker's control can cause the Traffic Management
    Microkernel (TMM) to terminate. Note: Software versions which have reached End of Technical Support (EoTS)
    are not evaluated. (CVE-2024-28889)

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://my.f5.com/manage/s/article/K000138912");
  script_set_attribute(attribute:"solution", value:
"Upgrade to one of the non-vulnerable versions listed in the F5 Solution K000138912.");
  script_set_attribute(attribute:"workaround_type", value:"config_change");
  script_set_attribute(attribute:"workaround", value:
"F5 lists a workaround with instructions listed at https://my.f5.com/manage/s/article/K000138912 that can be achieved using
the following steps:

  1. Configure the SSL Alert Timeout setting to Indefinite.
...
Tenable is not responsible for any negative effects that may occur from enacting this workaround.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2024-28889");
...
var vmatrix = {
  'AFM': {
    'affected': [
      '17.1.0-17.1.1','16.1.2.1-16.1.4','15.1.5-15.1.10'
    ],
    'unaffected': [
