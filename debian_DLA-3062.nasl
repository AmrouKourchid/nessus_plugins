...
advisory.
    ...
    uBlock, a Firefox add-on and efficient ads, malware and trackers blocker, supported an arbitrary depth of
    parameter nesting for strict blocking, which allows crafted web sites to cause a denial of service
    (unbounded recursion that can trigger memory consumption and a loss of all blocking functionality). Please
    note that webext-ublock-origin was replaced by webext-ublock-originfirefox. For Debian 9 stretch, this
    problem has been fixed in version 1.42.0+dfsg-1~deb9u1. We recommend that you upgrade your ublock-origin
    packages. For the detailed security status of ublock-origin please refer to its security tracker page at:
    https://security-tracker.debian.org/tracker/ublock-origin Further information about Debian LTS security
