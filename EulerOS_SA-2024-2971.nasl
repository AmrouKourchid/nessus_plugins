...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP11 : python3 (EulerOS-SA-2024-2971)");
...
the following vulnerabilities :
    ...
    The  socket module provides a pure-Python fallback to the socket.socketpair() function for platforms
    that dont support AF_UNIX, such as Windows. This pure-Python implementation uses AF_INET or AF_INET6 to
    create a local connected pair of sockets. The connection between the two sockets was not verified before
    passing the two sockets back to the user, which leaves the server socket vulnerable to a connection race
    from a malicious local peer.Platforms that support AF_UNIX such as Linux and macOS are not affected by
    this vulnerability. Versions prior to CPython 3.5 are not affected due to the vulnerable API not being
    included.(CVE-2024-3219)
...
number.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2024-2971
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?9e058f30");
...
var pkgs = [
  "python3-3.9.9-7.h30.eulerosv2r11",
  "python3-unversioned-command-3.9.9-7.h30.eulerosv2r11"
];
