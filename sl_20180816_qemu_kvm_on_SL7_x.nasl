...
if (description)
{
  ...
  script_name(english:"Scientific Linux Security Update : qemu-kvm on SL7.x x86_64 (20180816)");
...
"Security Fix(es) :
  ...
  - QEMU: i386: multiboot OOB access while loading kernel
    image (CVE-2018-7550)

Bug Fix(es) :

  - Previously, live migrating a Windows guest in some cases
    caused the guest to become unresponsive. This update
    ensures that Real-time Clock (RTC) interrupts are not
    missed, which prevents the problem from occurring.");
  # https://listserv.fnal.gov/scripts/wa.exe?A2=ind1808&L=scientific-linux-errata&F=&S=&P=1720
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?29c7e740");
...
flag = 0;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"qemu-img-1.5.3-156.el7_5.5")) flag++;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"qemu-kvm-1.5.3-156.el7_5.5")) flag++;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"qemu-kvm-common-1.5.3-156.el7_5.5")) flag++;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"qemu-kvm-debuginfo-1.5.3-156.el7_5.5")) flag++;
if (rpm_check(release:"SL7", cpu:"x86_64", reference:"qemu-kvm-tools-1.5.3-156.el7_5.5")) flag++;
