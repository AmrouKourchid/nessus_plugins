...
var appname = "VMware Workstation";
var ver = NULL;

registry_init();

var hklm = registry_hive_connect(hive:HKEY_LOCAL_MACHINE, exit_on_fail:TRUE);
var key = "SOFTWARE\VMware, Inc.\VMware Workstation\InstallPath";
...
var path = ereg_replace(pattern:"^(.+)\\$", replace:"\1", string:path);
var pver = hotfix_get_pversion(path:path + "\vmware.exe");
var err_res = hotfix_handle_error(
...
if (!isnull(ver))
{
  var matches = pregmatch(string:ver, pattern:"^([0-9.]+).*$", icase:TRUE);
...
if (isnull(ver)) audit(AUDIT_VER_FAIL, path + "\vmware.exe");
  ...
  set_kb_item(name:"Host/VMware Workstation/Version", value:ver);
