...
the USN-5071-2 advisory.
    ...
    USN-5071-1 fixed vulnerabilities in the Linux kernel for Ubuntu 20.04 LTS. This update provides the
    corresponding updates for the Linux Hardware Enablement (HWE) kernel from Ubuntu 20.04 LTS for Ubuntu
    18.04 LTS.
    ...
    It was discovered that the KVM hypervisor implementation for AMD processors in the Linux kernel did not
    ensure enough processing time was given to perform cleanups of large SEV VMs. A local attacker could use
    this to cause a denial of service (soft lockup). (CVE-2020-36311)
...
{
  audit(AUDIT_PATCH_INSTALLED, 'Kernel package for USN-5071-2');
}
