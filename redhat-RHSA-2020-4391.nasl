...
# extracted from Red Hat Security Advisory RHSA-2020:4391. The text
...
if (description)
{
  ...
  script_xref(name:"RHSA", value:"2020:4391");

  script_name(english:"RHEL 7 : openstack-cinder (RHSA-2020:4391)");

  script_set_attribute(attribute:"synopsis", value:
"The remote Red Hat host is missing a security update for openstack-cinder.");
  script_set_attribute(attribute:"description", value:
"The remote Redhat Enterprise Linux 7 host has packages installed that are affected by a vulnerability as referenced in
the RHSA-2020:4391 advisory.
    ...
    * Before this update, FC live migration was failing. With this update, the correct device information is
    now sent to os-brick for FC for the corresponding host. Also, the device is now removed from the correct
    masking view when the live migration process has failed on the Compute node. (BZ#1841157)

    * Before this update, the 3PAR driver did not look at the `_name_id` field for a possible volume ID, which
    caused volumes to be unusable after a live migration. With this update, the driver is now aware of the
    `_name_id` field as an alternative location for the volume ID, and live migrated volumes now work as
    expected. (BZ#1841866)

    * Before this update, the internal temporary snapshot, created during async migration when creating a
    volume from a snapshot, was not being deleted from the VNX storage.

    For example, if we create a new volume, V2, from snapshot S1, which we created from volume V1, an internal
    temporary snapshot, S2, is created from copying S1. V1 now has two snapshots, S1 and S2. Although we
    delete V1, V2 and S1 from OpenStack Block Storage (cinder), S2 is not deleted. This causes both V1 and S2
    to remain on the VNX storage.
...
number.");
  # https://security.access.redhat.com/data/csaf/v2/advisories/2020/rhsa-2020_4391.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?0f2b90aa");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/security/updates/classification/#moderate");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/errata/RHSA-2020:4391");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1741730");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1812988");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1841157");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1842748");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1843088");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1843196");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=1870569");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'openstack-cinder-12.0.10-19.el7ost', 'release':'7', 'el_string':'el7ost', 'rpm_spec_vers_cmp':TRUE, 'epoch':'1', 'exists_check':'openstack-'},
      {'reference':'python-cinder-12.0.10-19.el7ost', 'release':'7', 'el_string':'el7ost', 'rpm_spec_vers_cmp':TRUE, 'epoch':'1', 'exists_check':'openstack-'}
    ]
  }
];
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'openstack-cinder / python-cinder');
}
