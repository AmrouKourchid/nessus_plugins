...
# extracted from Amazon Linux 2 Security Advisory ALAS-2024-2458.
##
...
if (description)
{
  ...
  script_name(english:"Amazon Linux 2 : amazon-ssm-agent (ALAS-2024-2458)");

  script_set_attribute(attribute:"synopsis", value:
"The remote Amazon Linux 2 host is missing a security update.");
  script_set_attribute(attribute:"description", value:
"The version of amazon-ssm-agent installed on the remote host is prior to 3.2.2222.0-1. It is, therefore, affected by
multiple vulnerabilities as referenced in the ALAS2-2024-2458 advisory.
    ...
    responses from a Git server which triggers resource exhaustion in go-git clients.

    Applications using only the in-memory filesystem supported by go-git are not affected by this
    vulnerability.This is a go-git implementation issue and does not affect the upstream git cli.
    (CVE-2023-49568)
    ...
    Applications are only affected if they are using the  ChrootOS https://pkg.go.dev/github.com/go-git/go-
    billy/v5/osfs#ChrootOS , which is the default when using Plain versions of Open and Clone funcs (e.g.
    PlainClone). Applications using  BoundOS https://pkg.go.dev/github.com/go-git/go-billy/v5/osfs#BoundOS  or
    in-memory filesystems are not affected by this issue.This is a go-git implementation issue and does not
    affect the upstream git cli. (CVE-2023-49569)

Tenable has extracted the preceding description block directly from the tested product security advisory.

Note that Nessus has not tested for these issues but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://alas.aws.amazon.com/AL2/ALAS-2024-2458.html");
...
var pkgs = [
    {'reference':'amazon-ssm-agent-3.2.2222.0-1.amzn2', 'cpu':'aarch64', 'release':'AL2', 'rpm_spec_vers_cmp':TRUE},
    {'reference':'amazon-ssm-agent-3.2.2222.0-1.amzn2', 'cpu':'x86_64', 'release':'AL2', 'rpm_spec_vers_cmp':TRUE},
    {'reference':'amazon-ssm-agent-debuginfo-3.2.2222.0-1.amzn2', 'cpu':'aarch64', 'release':'AL2', 'rpm_spec_vers_cmp':TRUE},
    {'reference':'amazon-ssm-agent-debuginfo-3.2.2222.0-1.amzn2', 'cpu':'x86_64', 'release':'AL2', 'rpm_spec_vers_cmp':TRUE}
];
