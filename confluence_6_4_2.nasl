...
if (description)
{
  ...
  script_cve_id("CVE-2017-18086");
  script_bugtraq_id(103061);

  script_name(english:"Atlassian Confluence < 6.4.2 issuesURL Parameter Reflected XSS (CVE-2017-18086)");
...
Confluence application running on the remote host is prior to 6.4.2.
It is, therefore, affected by a reflected cross-site scripting
vulnerability in multiple resources that use the issuesURL parameter.

Note that Nessus has not tested for this issue but has instead relied
only on the application's self-reported version number.");
  script_set_attribute(attribute:"see_also", value:"https://jira.atlassian.com/browse/CONFSERVER-54907");
  script_set_attribute(attribute:"solution", value:
"Upgrade to Atlassian Confluence version 6.4.2 or later.");
...
if (version =~ '^6(\\.4)?$') audit(AUDIT_VER_NOT_GRANULAR, "Confluence", port, version);
