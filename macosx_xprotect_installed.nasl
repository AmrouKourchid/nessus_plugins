...
var info = get_av_info("macosx_xprotect");
if (isnull(info)) exit(1, "Failed to get Mac OS X XProtect info from antivirus.inc.");
var latest_defs_ver = make_array();
# First 3 indices deprecated - all supported OS X versions now use the same signature feed
latest_defs_ver[1] = info["latest_defs_ver[1]"];  # (inactive) Mac OS X 10.7
latest_defs_ver[2] = info["latest_defs_ver[2]"];  # (inactive) Mac OS X 10.8
latest_defs_ver[3] = info["latest_defs_ver[3]"];  # (inactive) Mac OS X 10.9
latest_defs_ver[4] = info["latest_defs_ver[4]"];  # ALL Mac OS X versions
...
var last_mod = get_kb_item(kb_base+"LastModification");
var defs = get_kb_item(kb_base+"DefinitionsVersion");
...
if (defs)
{
  if (preg(pattern:"^Mac OS X 1[0-3]\.[0-9]", string:os)) i = 4;
...
var spctl_stat = get_kb_item(kb_base+"spctl --status");
