...
if(description)
{
  ...
  script_name(english:"Trojan Horse Detection");
...
operating system be reinstalled to ensure removal.");
  ...
  script_copyright(english:"This script is Copyright (C) 2002-2025 and is owned by Tenable, Inc. or an Affiliate thereof.");

  script_dependencies("PC_anywhere_tcp.nasl", "SHN_discard.nasl", "X.nasl", "apcnisd_detect.nasl", "asip-status.nasl", "auth_enabled.nasl", "bugbear.nasl", "cifs445.nasl", "dcetest.nasl", "dns_server.nasl", "echo.nasl", "find_service1.nasl", "find_service2.nasl", "external_svc_ident.nasl", "mldonkey_telnet.nasl", "mssqlserver_detect.nasl", "mysql_version.nasl", "nessus_detect.nasl", "qmtp_detect.nasl", "radmin_detect.nasl", "rpc_portmap.nasl", "rpcinfo.nasl", "rsh.nasl", "socks.nasl", "telnet.nasl", "xtel_detect.nasl", "xtelw_detect.nasl");
...
# http://www.nessus.org/u?8227f344
# http://www.simovits.com/trojans/trojans.html
# and some antivirus websites
# 
trojanlist = "
...
TCP 447	Kraken
UDP 447	Kraken Command&Control
...
TCP 1000 Connecter, Der Sp?er / Der Spaeher, Direct Connection, Nibu
TCP 1001 Der Sp?er / Der Spaeher, Le Guardien, Silencer, WTheef, ebEx, Wortbot, Ghoice.12, Nibu
TCP 1005 Theef
TCP 1008 Lion
...
TCP 2000 Der Sp?er / Der Spaeher, Insane Network, Last 2000, Remote Explorer 2000, Senna Spy Trojan Generator, Feardoor, Fearic, CNK.A
UDP 2000 Fearic
TCP 2001 Der Sp?er / Der Spaeher, Trojan Cow, OICQSer.165, OICQSer.17
UDP 2001 Scalper Rootkit
TCP 2002 Singu
...
TCP 7718 Glacier
TCP 7777 God Message, The Thing (modified), Tini, Darkmoon, Arugizer
...
if ( os && "Windows" >!< os )
 exit(0, 'The remote operating system is not Windows.');

port = get_unknown_svc();
if (! port)
 exit(0, 'No unknown service.'); 

if (! service_is_unknown(port: port))
 exit(0, 'Service on port '+port+' is known.');
if (! get_port_state(port))
 exit(0, 'Port '+port+' is closed.');
# Only display non-SSL services
t = get_port_transport(port);
if (t != ENCAPS_IP )
 exit(0, 'Service on port '+port+' runs on SSL/TLS.');

req = strcat('^TCP[ \t]', port, '[ \t]');
str = egrep(string:trojanlist, pattern: req);
if (! str) exit(0, 'No Trojan horse is known on port '+port+'.');
...
if (report_paranoia < 2 || thorough_tests)
{
 ...
 if (! soc)
  exit(0, 'Connection refused on port '+port+'.');
...
name = ereg_replace(string: name, pattern: " *, *", replace: '\n\t');
m = string("An unknown service is running on this port.\n", 
	"It is sometimes opened by this/these Trojan horse(s):\n\t",
	name,"\n");
if (banner) m = strcat(m, 'Here is the service banner:\n', banner, '\n\n');
m = strcat(m, 'Unless you know for sure what service is behind it, you should\n',
