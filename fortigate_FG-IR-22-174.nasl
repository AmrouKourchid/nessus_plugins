...
if (description)
{
  ...
  script_name(english:"Fortinet Fortigate Read-Only users able to add/modify the Interface fields using the API (FG-IR-22-174)");
...
vulnerability as referenced in the FG-IR-22-174 advisory.

  - An improper access control [CWE-284] vulnerability in FortiOS version 7.2.0 and versions 7.0.0 through
    7.0.7 may allow a remote authenticated read-only user to modify the interface settings via the API.
    (CVE-2022-38380)
...
var constraints = [
  { 'min_version' : '6.4', 'max_version' : '6.4', 'fixed_version' : '7.0.8' },
