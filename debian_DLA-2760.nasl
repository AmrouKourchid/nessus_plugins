...
dla-2760 advisory.
    ...
    Multiple vulnerabilities were discovered in nettle, a low level cryptographic library, which could result
    in denial of service (remote crash in RSA decryption via specially crafted ciphertext, crash on ECDSA
    signature verification) or incorrect verification of ECDSA signatures. For Debian 9 stretch, these
    problems have been fixed in version 3.3-1+deb9u1. We recommend that you upgrade your nettle packages. For
    the detailed security status of nettle please refer to its security tracker page at: https://security-
    tracker.debian.org/tracker/nettle Further information about Debian LTS security advisories, how to apply
