...
# extracted from Red Hat Security Advisory RHSA-2024:9894. The text
...
if (description)
{
  ...
  script_xref(name:"RHSA", value:"2024:9894");

  script_name(english:"RHEL 8 / 9 : Red Hat Ansible Automation Platform 2.5 Product Release Update (Moderate) (RHSA-2024:9894)");
...
referenced in the RHSA-2024:9894 advisory.
    ...
    * Fix issue when port value isn't defined in the gateway_main_url variable (AAP-34716)
    ...
    * Fix issue not setting up gpg agent socket properly when multiple hub nodes are configured (AAP-34067)
    * ansible-automation-platform-installer and installer setup have been updated to 2.5-5
...
number.");
  ...
  # https://security.access.redhat.com/data/csaf/v2/advisories/2024/rhsa-2024_9894.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?0648228d");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/errata/RHSA-2024:9894");
  script_set_attribute(attribute:"solution", value:
"Update the affected ansible-core package.");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'ansible-core-2.16.13-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'epoch':'1', 'exists_check':'automation-hub-2.5'}
    ]
  },
  {
    'repo_relative_urls': [
    ...
    'pkgs': [
      {'reference':'ansible-core-2.16.13-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'epoch':'1', 'exists_check':'automation-hub-2.5'}
    ]
  }
];
...
else
{
  ...
  else audit(AUDIT_PACKAGE_NOT_INSTALLED, 'ansible-core');
}
