...
if (description)
{
  ...
  script_xref(name:"RHSA", value:"2023:4340");
  script_xref(name:"IAVA", value:"2023-A-0390");

  script_name(english:"RHEL 8 / 9 : Red Hat Ansible Automation Platform 2.4 Product Security and Bug Fix Update (Moderate) (RHSA-2023:4340)");
...
in the RHSA-2023:4340 advisory.
    ...
    * Topology view and Instances are only available as sidebar menu options to System Administrators and
    System Auditors. (AAP-11585)
    ...
    * Fixed an issue with slow database 'UPDATE' statements when using nested tasks(include_tasks) causing
    task manager timeout. (AAP-12586)
    * Added setting to enable queuing for Rsyslog to handle higher work
    volumes(LOG_AGGREGATOR_ACTION_MAX_DISK_USAGE_GB) (AAP-12726)
    ...
    * Fix for filter experience when assigning access to teams: In the dialog for granting teams to access a
    resource, it should be possible to filter teams using partial names. (AAP-13557)
    * Fixed bug where a weekly rrule string without a BYDAY would result in the UI throwing a TypeError
    (AAP-13670)
    * Fix server error that happens when deleting workflow jobs ran before event partitioning migration
    (AAP-13806)
    ...
    * Fix issue where related items were not visible in some cases: job template instance groups, organization
    galaxy credentials, and organization instance groups. (AAP-14057)

Tenable has extracted the preceding description block directly from the Red Hat Enterprise Linux security advisory.

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  # https://security.access.redhat.com/data/csaf/v2/advisories/2023/rhsa-2023_4340.json
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?af0a2e89");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/security/updates/classification/#moderate");
  script_set_attribute(attribute:"see_also", value:"https://bugzilla.redhat.com/show_bug.cgi?id=2226965");
  script_set_attribute(attribute:"see_also", value:"https://access.redhat.com/errata/RHSA-2023:4340");
...
var constraints = [
  {
    ...
    'pkgs': [
      {'reference':'automation-controller-4.4.1-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-cli-4.4.1-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-server-4.4.1-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-ui-4.4.1-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-venv-tower-4.4.1-1.el8ap', 'release':'8', 'el_string':'el8ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'}
    ]
  },
  {
    'repo_relative_urls': [
    ...
    'pkgs': [
      {'reference':'automation-controller-4.4.1-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-cli-4.4.1-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-server-4.4.1-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-ui-4.4.1-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'},
      {'reference':'automation-controller-venv-tower-4.4.1-1.el9ap', 'release':'9', 'el_string':'el9ap', 'rpm_spec_vers_cmp':TRUE, 'exists_check':'automation-hub-2.4'}
    ]
  }
];
