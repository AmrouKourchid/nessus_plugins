...
if (description)
{
  ...
  script_name(english:"Ubuntu 18.04 LTS / 20.04 LTS / 22.04 LTS : curl vulnerabilities (USN-5412-1)");
...
vulnerabilities as referenced in the USN-5412-1 advisory.
    ...
    Axel Chong discovered that curl incorrectly handled percent-encoded URL separators. A remote attacker
    could possibly use this issue to trick curl into using the wrong URL and bypass certain checks or filters.
    This issue only affected Ubuntu 22.04 LTS. (CVE-2022-27780)

    Florian Kohnhuser discovered that curl incorrectly handled returning a TLS server's certificate chain
    ...
    Harry Sintonen discovered that curl incorrectly reused a previous connection when certain options had been
    changed, contrary to expectations. (CVE-2022-27782)
