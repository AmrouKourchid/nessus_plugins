...
if (description)
{
  ...
  script_name(english:"PHP 7.3.x < 7.3.1 Multiple vulnerabilities.");
...
server is 7.3.x prior to 7.3.1. It is, therefore, affected by
multiple vulnerabilities:
  ...
  targeted system, to disclose sensitive information.
  (CVE-2019-9021)
  ...
  attacker to completely compromise the target system.
  (CVE-2019-9023)
  ...
  - An out-of-bounds read and write error exists in the mb_split
  function due to improper implementation of memory operations. An
  unauthenticated, remote attacker can exploit this by causing PHP to
  execute the memcpy function with a negative argument, to cause 
  buffer over-read and over-write conditions which could allow the
  attacker to completely compromise the system.
  (CVE-2019-9025)");
  script_set_attribute(attribute:"see_also", value:"http://php.net/ChangeLog-7.php#7.3.1");
