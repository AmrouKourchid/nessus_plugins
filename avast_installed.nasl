...
var file = hotfix_append_path(path:path, value:exe);
...
else
{
  ...
  var pattern = "Latest=([0-9]+)";
  var item = pregmatch(pattern:pattern, string:data);
  if (!isnull(item))
  {
    avdefs = item[1];
...
{
  var programdata = hotfix_get_programdata();
  var info = hotfix_append_path(path:programdata, value:"\AVAST Software\Avast\avast5.ini");
...
# Generate report
var report = "The Avast Antivirus System is installed on the remote host :
...
if ( avdefs != 'unknown' )
{
  var match = pregmatch(pattern:"^(\d{2})(\d{2})(\d{2})\d+$", string:avdefs);
