...
affected by multiple vulnerabilities as referenced in the K91589041 advisory.

    CVE-2021-45960In Expat (aka libexpat) before 2.4.3, a left shift by 29 (or more) places in the storeAtts
    function in xmlparse.c can lead to realloc misbehavior (e.g., allocating too few bytes, or only freeing
    memory).CVE-2022-22825lookup in xmlparse.c in Expat (aka libexpat) before 2.4.3 has an integer
    overflow.CVE-2022-22826nextScaffoldPart in xmlparse.c in Expat (aka libexpat) before 2.4.3 has an integer
    overflow.CVE-2022-22827storeAtts in xmlparse.c in Expat (aka libexpat) before 2.4.3 has an integer
    overflow.ImpactA remote attacker could send specially crafted XML which, when parsed by an application
    using the Expat library, would result in a buffer over-read and cause the application to stop responding.
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://my.f5.com/manage/s/article/K91589041");
