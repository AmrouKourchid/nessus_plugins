...
var app = "Symantec Antivirus";
var cpe = "cpe:/a:symantec:antivirus";
...
var key = "SOFTWARE\Symantec\InstalledApps\";
var item = "SAVCE";
var item2 = "NAV";
...
var signature_array = check_signature_version();

if (isnull(signature_array))
  exit(1, "Unable to obtain signature information from the registry.");

var current_signature_version  = signature_array[0];
var current_signature_path  = signature_array[1];
var current_signature_registry  = signature_array[2];
var current_signature_version_full = signature_array[3];
var update_enabled = signature_array[4];
...
#-------------------------------------------------------------#
sep = 0;
var product_version = check_product_version();

#-------------------------------------------------------------#
# Checks the product name                                     #
#-------------------------------------------------------------#
var product_name = check_product_name();

#-------------------------------------------------------------#
# Checks the product type (if applicable) and                 #
# Check if a hotfix has been applied to the host              #
#-------------------------------------------------------------#
var product_type, hotfix_applied;
if (sep)
{
...
#
var report = "The remote host has antivirus software from Symantec installed. It has
...
#
var apply_date = NULL;
var logpath = 'c:\\ProgramData\\Symantec\\LiveUpdate\\Log.LiveUpdate';
var log = hotfix_get_file_contents(logpath);
var err = hotfix_handle_error(error_code:log['error'], file:logpath);
var formatted_def, log_line;
if(err)
