...
if (description)
{
  ...
  script_name(english:"Apache HTTP Server SEoL (2.1.x <= x <= 2.2.x)");

  script_set_attribute(attribute:"synopsis", value:
"An unsupported version of Apache HTTP Server is installed on the remote host.");
  script_set_attribute(attribute:"description", value:
"According to its version, Apache HTTP Server is between 2.1.x and 2.2.x. It is, therefore, no longer maintained by its
vendor or provider.

Lack of support implies that no new security patches for the product will be released by the vendor. As a result, it may
contain security vulnerabilities.");
  script_set_attribute(attribute:"see_also", value:"https://archive.apache.org/dist/httpd/Announcement2.2.txt");
...
"Upgrade to a version of Apache HTTP Server that is currently supported.");
  ...
  script_set_attribute(attribute:"seol_date", value:"2017/07/11");
...
var constraints = [
  { max_branch : '2.2', min_branch : '2.1', seol : 20170711 }
];
