...
foreach dir (dirs)
{
  ...
  if ((("200 OK" >!< loginres[0]) || ("HTTP/1.1 200" >!< loginres[0])) && ("X-Jenkins:" >< res[1]) || ("X-Hudson:" >< res[1]) || ("Jenkins-Version" >< res[2]))
  {
  ...
  {
    if (("Jenkins-Version" >< res[1]) || ("Jenkins-Version" >< res[2]))
    {
      ...
      if (!isnull(item)) jenkins_ver = item[1];
        else item = pregmatch(pattern:"Jenkins-Version:\s([0-9]+\.[0-9.]+)", string: res[2]);
        if (!isnull(item)) jenkins_ver = item[1];
