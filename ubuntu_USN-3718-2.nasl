...
if (description)
{
  ...
  script_name(english:"Ubuntu 16.04 LTS : Linux kernel (HWE) regression (USN-3718-2)");
...
USN-3718-2 advisory.
    ...
    USN-3695-2 fixed vulnerabilities in the Linux Hardware Enablement Kernel (HWE) kernel for Ubuntu 16.04
    LTS. Unfortunately, the fix for CVE-2018-1108 introduced a regression where insufficient early entropy
    prevented services from starting, leading in some situations to a failure to boot, This update addresses
    the issue.
...
number.");
  script_set_attribute(attribute:"see_also", value:"https://ubuntu.com/security/notices/USN-3718-2");
...
{
  audit(AUDIT_PATCH_INSTALLED, 'Kernel package for USN-3718-2');
}
