...
var cpe = "cpe:/a:pandasecurity:panda_antivirus";
var app_name = 'Panda Security Antivirus';
...
var prod_subkeys = make_array();
var name_subkeys = make_array();
var path_subkeys = make_array();
var ver_subkeys = make_array();
...
var prod, key, key_h, name, value, path, ver, extra;
...
if (!isnull(path))
{
  ...
  var sigfile =  ereg_replace(pattern:"^[A-Za-z]:(.*)", replace:"\1\pav.sig", string:path);
  ...
  if (rc == 1) {
    ...
    if (!isnull(fh)) {
      var day, month, year, sigs_time;
      var hex_date = ReadFile(handle:fh, offset:0x65, length:4);
      if (!isnull(hex_date)) {
    ...
    {
      var dirpath, timestamp, file, file_name, file_date;
...
# - general info.
var report = "Panda Antivirus is installed on the remote host :
...
if (sigs_target =~ "^[0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]") {
  var a = split(sigs_target, sep:"-", keep:0);
  var sigs_target_yyyymmdd = a[2] + a[0] + a[1];
  if (int(sigs_target_yyyymmdd) < ( int(sigs_vendor_yyyymmdd) - 1 )) {
    var sigs_vendor_mmddyyyy = 
...
# build a filtered list of panda related services
var panda_svc = pgrep(pattern:"Panda|panda", string:services);
dbg::detailed_log(lvl:3, msg:"Running services that are potentially related to Panda Antivirus: "+'\n'+panda_svc+'\n');
...
if (services)
{
  ...
  if (
    ...
    (
      ...
      "panda cloud antivirus service" >< services
    ) &&
    ...
    (
      ...
      # Panda Endpoint Protection
      "panda endpoint administration agent" >< services || 
      
      "panda devices agent" >< services ||

      "panda endpoint agent" >< services ||

      "panda elam service protection" >< services 
    )
  ) running = TRUE;
...
extra['Antivirus Installed'] = name;
