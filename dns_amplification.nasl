...
var packet = mkdns(dns:dns, query:mk_query(txt:mk_query_txt(""),type:0x0002, class:0x0001));

var out_len = strlen(packet);
...
var rsp = recv(socket:soc, length:4096);
dbg::log(msg:'Incoming Packet :', ddata:rsp);

close(soc);

var in_len = strlen(rsp);
