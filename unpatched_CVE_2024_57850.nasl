...
if (description)
{
  ...
  script_cve_id("CVE-2024-57850");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2024-57850");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: jffs2: Prevent rtime decompress memory
    corruption The rtime decompression routine does not fully check bounds during the entirety of the
    decompression pass and can corrupt memory outside the decompression buffer if the compressed data is
    corrupted. This adds the required check to prevent this failure mode. (CVE-2024-57850)
