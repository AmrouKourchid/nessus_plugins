...
if (description)
{
  ...
  script_name(english:"Fortinet Fortigate Information disclosure in web proxy error pages (FG-IR-21-231)");
...
vulnerability as referenced in the FG-IR-21-231 advisory.

  - A server-generated error message containing sensitive information in Fortinet FortiOS 7.0.0 through 7.0.3,
    6.4.0 through 6.4.8, 6.2.x, 6.0.x and FortiProxy 7.0.0 through 7.0.1, 2.0.x allows malicious webservers to
    retrieve a web proxy's client username and IP via same origin HTTP requests triggering proxy-generated
    HTTP status codes pages. (CVE-2021-43206)
...
"For 6.0.x, see vendor advisory. For 6.2.x, upgrade to Fortigate version 6.2.11 or later. For 6.4.x, upgrade to Fortigate
version 6.4.10 or later. For 7.0.x, upgrade to Fortigate version 7.0.4 or later.");
