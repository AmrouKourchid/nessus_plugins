...
if (description)
{
  ...
  script_name(english:"Ubuntu 16.04 LTS / 18.04 LTS / 20.04 LTS : Sudo vulnerabilities (USN-4705-1)");
...
vulnerabilities as referenced in the USN-4705-1 advisory.
    ...
    It was discovered that Sudo incorrectly handled memory when parsing command lines. A local attacker could
    possibly use this issue to obtain unintended access to the administrator account. (CVE-2021-3156)

    It was discovered that the Sudo sudoedit utility incorrectly handled checking directory permissions. A
    local attacker could possibly use this issue to bypass file permissions and determine if a directory
    exists or not.
