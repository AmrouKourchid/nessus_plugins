...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP3 : pki-core (EulerOS-SA-2021-1831)");
...
vulnerabilities :
  ...
  - A flaw was found in pki-core. An attacker who has
    successfully compromised a key could use this flaw to
    renew the corresponding certificate over and over
    again, as long as it is not explicitly revoked. The
    highest threat from this vulnerability is to data
    confidentiality and integrity.(CVE-2021-20179)
  ...
  - A flaw was found in pki-core. A specially crafted POST
    request can be used to reflect a DOM-based cross-site
    scripting (XSS) attack to inject code into the search
    query form which can get automatically executed. The
    highest threat from this vulnerability is to data
    integrity.(CVE-2020-25715)

  - A Reflected Cross Site Scripting vulnerability was
    found in all pki-core 10.x.x versions, where the pki-ca
    module from the pki-core server. This flaw is caused by
    missing sanitization of the GET URL parameters. An
    attacker could abuse this flaw to trick an
    authenticated user into clicking a specially crafted
    link which can execute arbitrary code when viewed in a
    browser.(CVE-2019-10221)
  ...
  - A flaw was found in the Key Recovery Authority (KRA)
    Agent Service where it did not properly sanitize the
    recovery ID during a key recovery request, enabling a
...
without introducing additional issues.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2021-1831
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?14b99600");
...
pkgs = ["pki-base-10.4.1-11.h7",
        "pki-base-java-10.4.1-11.h7",
        "pki-ca-10.4.1-11.h7",
        "pki-kra-10.4.1-11.h7",
        "pki-server-10.4.1-11.h7",
        "pki-symkey-10.4.1-11.h7",
        "pki-tools-10.4.1-11.h7"];
