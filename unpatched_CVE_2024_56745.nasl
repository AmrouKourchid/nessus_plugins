...
if (description)
{
  ...
  script_cve_id("CVE-2024-56745");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2024-56745");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: PCI: Fix reset_method_store() memory
    leak In reset_method_store(), a string is allocated via kstrndup() and assigned to the local options.
    options is then used in with strsep() to find spaces: while ((name = strsep(&options,  )) != NULL) { If
    there are no remaining spaces, then options is set to NULL by strsep(), so the subsequent kfree(options)
    doesn't free the memory allocated via kstrndup(). Fix by using a separate tmp_options to iterate with
    strsep() so options is preserved. (CVE-2024-56745)
...
"There is no known solution at this time.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2024-56745");
