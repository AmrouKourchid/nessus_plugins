...
if (description)
{
  ...
  script_cve_id("CVE-2001-1530");

  script_name(english:"Webmin 0.80 / 0.88 world-writable files");

  script_set_attribute(attribute:"synopsis", value:
"The remote web server is affected by a temporary file creation vulnerability.");
  script_set_attribute(attribute:"description", value:
"According to its self-reported version, the Webmin install hosted on
the remote host is 0.80 or 0.88. It is, therefore, affected by an
issue in run.cgi which allows for the creation of temporary files
with world-writable permissions, which could lead to arbitrary code
execution.");
...
if (
    ver_compare(ver:version, fix:"0.80", strict:FALSE) == 0 ||
    ver_compare(ver:version, fix:"0.88", strict:FALSE) == 0
  )
{
  report =
    '\n  URL               : ' + install_url +
