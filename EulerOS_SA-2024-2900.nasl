...
if (description)
{
  ...
  script_name(english:"EulerOS 2.0 SP10 : c-ares (EulerOS-SA-2024-2900)");
...
the following vulnerabilities :
    ...
    configuration files such as `/etc/resolv.conf`, `/etc/ nsswitch.conf`, the `HOSTALIASES` file, and if
    using a c-ares version prior to 1.27.0, the `/etc/hosts` file.If any of these configuration files has an
    embedded `NULL` character as the first character in a new line, it can lead to attempting to read memory
    prior to the start of the given buffer which may result in a crash.This issue is fixed in c-ares 1.27.0.No
    known workarounds exist.(CVE-2024-25629)
...
number.");
  # https://developer.huaweicloud.com/ict/en/site-euleros/euleros/security-advisories/EulerOS-SA-2024-2900
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?837e072f");
...
var pkgs = [
  "c-ares-1.16.1-1.h7.eulerosv2r10"
];
