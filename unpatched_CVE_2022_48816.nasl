...
if (description)
{
  ...
  script_cve_id("CVE-2022-48816");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2022-48816");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: SUNRPC: lock against ->sock changing
    during sysfs read ->sock can be set to NULL asynchronously unless ->recv_mutex is held. So it is important
    to hold that mutex. Otherwise a sysfs read can trigger an oops. Commit 17f09d3f619a (SUNRPC: Check if the
    xprt is connected before handling sysfs reads) appears to attempt to fix this problem, but it only
    narrows the race window. (CVE-2022-48816)
...
"There is no known solution at this time.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2022-48816");
