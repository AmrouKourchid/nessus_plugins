...
# extracted from Ubuntu Security Notice USN-5921-1. The text
...
if (description)
{
  ...
  script_xref(name:"USN", value:"5921-1");

  script_name(english:"Ubuntu 18.04 LTS / 20.04 LTS / 22.04 LTS : rsync vulnerabilities (USN-5921-1)");
...
referenced in the USN-5921-1 advisory.

    Koen van Hove discovered that the rsync client incorrectly validated filenames returned by servers. If a
    user or automated system were tricked into connecting to a malicious server, a remote attacker could use
    this issue to write arbitrary files, and possibly escalate privileges.

Tenable has extracted the preceding description block directly from the Ubuntu security advisory.

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://ubuntu.com/security/notices/USN-5921-1");
...
var pkgs = [
    {'osver': '18.04', 'pkgname': 'rsync', 'pkgver': '3.1.2-2.1ubuntu1.6'},
    {'osver': '20.04', 'pkgname': 'rsync', 'pkgver': '3.1.3-8ubuntu0.5'},
    {'osver': '22.04', 'pkgname': 'rsync', 'pkgver': '3.2.7-0ubuntu0.22.04.2'}
];
