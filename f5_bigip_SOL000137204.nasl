...
# extracted from F5 Networks BIG-IP Solution K000137204.
#
# @NOAGENT@
##
...
if (description)
{
  ...
  script_name(english:"F5 Networks BIG-IP : Intel BIOS vulnerability (K000137204)");

  script_set_attribute(attribute:"synopsis", value:
"The remote device is missing a vendor-supplied security patch.");
  script_set_attribute(attribute:"description", value:
"The version of F5 Networks BIG-IP installed on the remote host is prior to tested version. It is, therefore, affected by
a vulnerability as referenced in the K000137204 advisory.

  - Insufficient control flow management in the BIOS firmware for some Intel(R) Processors may allow a
    privileged user to potentially enable denial of service via local access. (CVE-2022-43505)

Note that Nessus has not tested for this issue but has instead relied only on the application's self-reported version
number.");
  script_set_attribute(attribute:"see_also", value:"https://my.f5.com/manage/s/article/K000137204");
...
if (bigip_is_affected(vmatrix:vmatrix, sol:sol))
{
  ...
  var cmd = "show sys hardware";
  var out = ldnix::run_cmd_template_wrapper(template:cmd);
  
  if (info_t == INFO_SSH) ssh_close_connection();
  if (empty_or_null(out)) audit(AUDIT_HOST_NOT, 'responding to command "show sys hardware"');
  # output example at : my[dot]f5[dot]com/manage/s/article/K13144
  if (out !~ "(^|\s)Name\s+BIG-IP\s+(i2000|i4000)($|\s)")
