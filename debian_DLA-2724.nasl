...
advisory.
    ...
    HTCondor, a distributed workload management system, has Incorrect Access Control. It is possible to use a
    different authentication method to submit a job than the administrator has specified. If the administrator
    has configured the READ or WRITE methods to include CLAIMTOBE, then it is possible to impersonate another
    user to the condor_schedd, for example to submit or remove jobs. For Debian 9 stretch, this problem has
    been fixed in version 8.4.11~dfsg.1-1+deb9u1. We recommend that you upgrade your condor packages. For the
    detailed security status of condor please refer to its security tracker page at: https://security-
    tracker.debian.org/tracker/condor Further information about Debian LTS security advisories, how to apply
