...
"It is, therefore, affected by multiple vulnerabilities as referenced in the ALAS2023-2023-032 advisory.
    ...
    It was found that the Kerberos Key Distribution Center (KDC) delegation feature, Service for User (S4U),
    did not sufficiently protect the tickets it's providing from tempering. A malicious, authenticated service
    principal allowed to delegate could use this flaw to impersonate a non-forwardable user. (CVE-2020-17049)
    ...
    A fix for CVE-2021-20316 will not be provided for Amazon Linux 1 and Amazon Linux 2. You can mitigate this
    issue by disabling SMBv1.To do so, add server min protocol = SMB2 to the [global] section of
    /etc/samba/smb.conf (CVE-2021-20316)
    ...
    A fix for CVE-2021-44141 for will not be provided for Amazon Linux 1 and Amazon Linux 2. You can mitigate
    this issue by disabling SMBv1.To do so, add server min protocol = SMB2 to the [global] section of
    /etc/samba/smb.conf (CVE-2021-44141)
    ...
    Samba 4.17 introduced following symlinks in user space with the intent to properly check symlink targets
    to stay within the share that was configured by the administrator. The check does not properly cover a
    corner case, so that a user can create a symbolic link that will make smbd escape the configured share
    path. (CVE-2022-3592)
