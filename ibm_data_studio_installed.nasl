...
environment."
  ...
  script_set_attribute(attribute:"see_also", value:"https://www.ibm.com/developerworks/downloads/im/data/");
...
item = pregmatch(pattern:'-input\\s*\"(.+\\\\)uninstall\\\\', string:uninstall_path);
...
foreach var file (make_list(notice_file, notice_file1))
{
  fh = CreateFile(
    file:file,
  ...
  if (fsize)
  {
    ...
    {
      item = pregmatch(pattern:'ibm data studio client v([0-9.]+)',
                       string:data);
      if (!isnull(item)) version = item[1];

      # detect "full" client and other variants
      item = pregmatch(pattern:'ibm data studio [^\n]+ v([0-9.]+)',
                       string:data);
      if (!isnull(item)) version = item[1];
    }
  }
