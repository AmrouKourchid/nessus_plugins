...
# extracted from F5 Networks BIG-IP Solution K16136.
#
...
if (description)
{
  ...
  script_cve_id("CVE-2014-8275");
  script_bugtraq_id(71935);

  script_name(english:"F5 Networks BIG-IP : OpenSSL vulnerability (K16136)");
...
crypto/x509/x_all.c. (CVE-2014-8275)

Impact

This vulnerability does not allow an attacker to forge certificates,
and does not affect certificate verification or OpenSSL
servers/clients in any other way. It also does not affect common
revocation mechanisms. Only custom applications that rely on the
uniqueness of the fingerprint (for example, certificate denylists) may
be affected.");
  script_set_attribute(attribute:"see_also", value:"https://support.f5.com/csp/article/K16136");
