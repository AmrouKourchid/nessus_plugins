...
timestamp replies (14)." );
 ...
 script_copyright(english:"This script is Copyright (C) 1999-2024 Tenable Network Security, Inc.");
...
# Change to 1 for NASL CLI debugging
var cli_debug = 0;
...
var icmp = forge_icmp_packet(ip:ip,icmp_type : 13, icmp_code:0,
                          icmp_seq : 1, icmp_id : 1);
			  
var filter = strcat("icmp and src host ", get_host_ip(), " and dst host ", compat::this_host(), " and icmp[0:1] = 14");

# decalre all vars in for loop below
var i, rep, type, code, txt, data, x1, x2, x3 ,x4 ,e ,eB ,eL ,non_std1 ,non_std4 ,txt2 ,t1B ,t1L ,t2 ,v, s;
var fail_count = 0;
...
for(i=0;i<3;i++)
{
 ...
 if(rep)
 {
  ...
  dbg::detailed_log(
    lvl:2, 
    msg:'Checking ICMP packet type and code:',
    msg_details:{
    'type :':{'lvl':2, 'value': type},
    'code :':{'lvl':2, 'value': code}
  });
  ...
  if(type == 14 && code == 0)
  {
   ...
   if (data)
   {
    ...
    dbg::detailed_log(
      lvl:2, 
      msg:'Checking timestamps:',
      msg_details:{
        'data :':{'lvl':2, 'value': hexstr(data)},
        'x1   :':{'lvl':2, 'value': x1},
        'x2   :':{'lvl':2, 'value': x2},
        'x3   :':{'lvl':2, 'value': x3},
        'x4   :':{'lvl':2, 'value': x4}
    ...
    if (t1B >= 0 && t1B < 86401 || t1L >= 0 && t1L < 86401)
    {
     ...
     dbg::detailed_log(
      lvl:2, 
      msg:'Checking timestamps, continued:',
      msg_details:{
      'eB :':{'lvl':2, 'value': eB},
      'eL :':{'lvl':2, 'value': eL},
      'e  :':{'lvl':2, 'value': e}
  ...
  security_report_v4(port: 0, proto:'icmp' , severity:SECURITY_NOTE , extra:txt);
  exit(0);
 }
 else 
 {
    dbg::detailed_log(
      lvl:2, 
      msg:'Target did not respond to ICMP request #' + i
    );
    fail_count++;
 }
}
...
if (fail_count);
  exit(0, 'Target did not respond to ICMP.');
