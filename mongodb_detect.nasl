...
# mongoDB v4.2 introduced TLS controls to replace SSL controls. Queries may need to be wrapped in TLS to continue
if (isnull(response))
{
  dbg::detailed_log(lvl:3, src:SCRIPT_NAME, msg:
    'Port ' + port + ': Initial query_ismaster response is NULL, attempting to negotiate TLSv1.2 and executing query again');
  soc = open_sock_tcp(port, transport:ENCAPS_IP);
  soc = socket_negotiate_ssl(socket:soc, transport:ENCAPS_TLSv1_2 | ENCAPS_DISABLE_SSLv2 | ENCAPS_DISABLE_SSLv3, async:FALSE);
  send(socket:soc, data:query_ismaster);
...
{
  dbg::detailed_log(lvl:3, src:SCRIPT_NAME, msg:
    'Port ' + port + ': Subsequent TLS-wrapped query_ismaster response is NULL, leveraging SSL cert auth');
  close(soc);
