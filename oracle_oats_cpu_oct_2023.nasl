...
if (description)
{
  ...
  script_name(english:"Oracle Application Testing Suite DoS (October 2023 CPU)");
...
vulnerability as referenced in the October 2023 CPU advisory:

  - Vulnerability in the Oracle Application Testing Suite product of Oracle Enterprise Manager (component: Load 
    Testing for Web Apps (Google Gson)). The supported version that is affected is 13.3.0.1. Easily exploitable 
    vulnerability allows unauthenticated attacker with network access via HTTP to compromise Oracle Application 
    Testing Suite. Successful attacks of this vulnerability can result in unauthorized ability to cause a hang 
    or frequently repeatable crash (complete DOS) of Oracle Application Testing Suite. (CVE-2022-25647)
...
{
  patches_to_report = make_list('35878825');
}
else
{
  patches_to_report = make_list('35878825', '34395275');
  patches_to_check = make_list('34395275');
}


var constraints = [
  { 'min_version' : '13.3.0.1', 'fixed_version' : '13.3.0.1.576' }
];
