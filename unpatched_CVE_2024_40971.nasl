...
if (description)
{
  ...
  script_cve_id("CVE-2024-40971");

  script_name(english:"Linux Distros Unpatched Vulnerability : CVE-2024-40971");
...
patch available.
  ...
  - In the Linux kernel, the following vulnerability has been resolved: f2fs: remove clear SB_INLINECRYPT flag
    in default_options In f2fs_remount, SB_INLINECRYPT flag will be clear and re-set. If create new file or
    open file during this gap, these files will not use inlinecrypt. Worse case, it may lead to data
    corruption if wrappedkey_v0 is enable. Thread A: Thread B: -f2fs_remount -f2fs_file_open or f2fs_new_inode
    -default_options <- clear SB_INLINECRYPT flag -fscrypt_select_encryption_impl -parse_options <- set
    SB_INLINECRYPT again (CVE-2024-40971)
...
"There is no known solution at this time.");
  ...
  script_set_attribute(attribute:"cvss_score_source", value:"CVE-2024-40971");
