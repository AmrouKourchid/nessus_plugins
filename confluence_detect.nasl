...
function confluence_request(method, port, item, data)
{
  ...
  if (isnull(res)) dbg::detailed_log(lvl:1, msg:'The web server on port '+port+' failed to respond.');
...
if (!empty_or_null(user) && !empty_or_null(pass))
{
  ...
  {
    secure = FALSE;
    dbg::detailed_log(lvl:1, msg:'An authenticated request to Confluence has failed because the HTTP transport is insecure.');
  }
  ...
  if (secure)
  {
    ...
    else dbg::detailed_log(lvl:1, msg:'Could not obtain a valid auth token for this session.');
    ...
    if (!empty_or_null(sess_token))
    {
      ...
      else dbg::detailed_log(lvl:1, msg:'The version information from the about page is missing or invalid.');
      ...
      else dbg::detailed_log(lvl:1, msg:'The build information could not be obtained during authenticated request.');
    ...
    else dbg::detailed_log(lvl:1, msg:'The session token is either missing or invalid.');
  }
}
...
if (!detected)
{
  foreach path (paths)
  {
    dbg::detailed_log(lvl:1, msg:'Checking path: ' + path);
    detected = FALSE; 
    ...
    foreach var item (items)
    {
      ...
      dbg::detailed_log(lvl:1, msg:'Checking item: ' + item);
      ...
      if (!empty_or_null(body))
      {
        ...
        if (!footer_content) dbg::detailed_log(lvl:1, msg:"Missing Confluence footer in '" + item + "'.");

        foreach var pat (canon_url_pats)
        {
          dbg::detailed_log(lvl:1, msg:'Trying regex: ' + pat);
          line = pgrep(string:body, pattern:pat);
          if (!empty_or_null(line))
          {
            dbg::detailed_log(lvl:1, msg:'Successfully matched a canonical URL.');
            break;
          }
        }
      }
      else dbg::detailed_log(lvl:1, msg:"Empty or null response returned from request to '" + item + "'.");

      if (!empty_or_null(line)) matches = pregmatch(string:line, pattern:'action="(?:https?://)?[^/]*(.*)/dologin\\.action"');
      else dbg::detailed_log(lvl:1, msg:"None of the canonical URLs matched in '" + item + "'.");
      
      if (isnull(matches))
      {
        dbg::detailed_log(lvl:1, msg:"Failed to parse the path from '" + item + "'.");
        dbg::detailed_log(lvl:1, msg:'Trying legacy pat: ' + legacy_pat);
      ...
      {
        dbg::detailed_log(lvl:1, msg:"The canonical URL did not match the expected base URL in '" + item + "'.");
        continue;
      }

      if (empty_or_null(matches))
      {
        dbg::detailed_log(lvl:1, msg:'Failed to detect confluence at: ' + item);
        continue;
      }
      dbg::detailed_log(lvl:1, msg:'Successfully detected confluence. Trying to get version');
      
      # Get version
      foreach var ver_pat (ver_pats)
      {
        dbg::detailed_log(lvl:1, msg:'Checking version with regex: ' + ver_pat);
        if (!empty_or_null(body)) matches = pregmatch(pattern:ver_pat, string:body, icase:FALSE);
        if (isnull(matches))
        {
          dbg::detailed_log(lvl:1, msg:'Did not find any matches for version regex');
          continue;
        }
        dbg::detailed_log(lvl:1, msg:'Successfully found version');
      ...
      if (build == UNKNOWN_VER || empty_or_null(build))
      {
        ...
        else dbg::detailed_log(lvl:1, msg:"Could not obtain build information from '" + item + "'.");
