...
if (description)
{
  ...
  script_name(english:"F5 BIG-IP Edge Client Windows Component Installer < 7.2.4.1 MITM (K000132522)");

  script_set_attribute(attribute:"synopsis", value:
"A web client installed on the remote Windows host is affected by a man in the middle
vulnerability.");
  script_set_attribute(attribute:"description", value:
"The version of the Big-IP Edge Client Windows Component Installer installed on the remote Windows
host is before 7.2.4.1. In the pre connection stage, an improper enforcement of message integrity vulnerability exists 
in BIG-IP Edge Client for Windows. An unauthenticated attacker with a man-in-the-middle position between the BIG-IP 
Edge Client and BIG-IP APM may exploit this vulnerability to modify requests and responses to and from the BIG-IP Edge 
Client.
