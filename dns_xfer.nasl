...
information.");
 ...
 script_set_attribute(attribute:"cvss_score_source", value:"CVE-1999-0532");
 script_set_attribute(attribute:"cvss_score_rationale", value:"Tenable gives a Confidentiality impact of Partial since the issue could reveal useful info to an attacker.");
...
"2001/01/16");
 ...
 script_copyright(english:"This script is Copyright (C) 2001-2025 Tenable Network Security, Inc.");
 script_family(english: "DNS");

 script_require_ports("Services/dns", 53);
 script_dependencies("dns_server.nasl", "smtpserver_detect.nasl", "ssl_cert_CN_mismatch.nasl", "snmp_sysDesc.nasl", "netbios_name_get.nasl", "alternate_hostnames.nasl");
 exit(0);
}
...
# ...via the SSL cert, SNMP, SMB, or alternate_hostnames.nasl
foreach kb_label ( make_list('X509/*/altName', 'SNMP/sysName', 'SMB/name', 'Host/alt_name' ) )
{
...
# New installs sometimes have some built-in zones
var default_names = [
  'localhost',
  'localdomain'
];

foreach hn (default_names)
  domains = make_list(domains, get_domains_from_hostname(hn));
...
dbg::detailed_log(lvl:2, msg:'Found some potential domain names...');

foreach domain ( domains )
{
 dbg::detailed_log(lvl:2, msg:'Checking ' + domain);
 ...
 dns = dns_split(pkt);
 dbg::detailed_log(lvl:2, msg:'DNS RCODE: ' + DNS_RCODES[dns['flags'] & 0x000F]);
