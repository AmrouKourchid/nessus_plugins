...
if (description)
{
  ...
  script_name(english:"Amazon Linux 2022 : freerdp, freerdp-devel, freerdp-libs (ALAS2022-2022-082)");
...
"It is, therefore, affected by multiple vulnerabilities as referenced in the ALAS2022-2022-082 advisory.
    ...
    A flaw was found in the FreeRDP client when it fails to validate input data when using gateway
    connections. This flaw could allow a malicious gateway to send a specially crafted input to a client
    leading to an out of bounds write in client memory. The highest threat from this flaw is that it could
    allow arbitrary code to be executed on the target system. (CVE-2021-41159)

    A flaw was found in the FreeRDP client where it fails to validate input data when using connections with
    GDI or SurfaceCommands. This flaw could allow a malicious server sending graphics updates to a client to
    cause an out of bounds write in client memory using a specially crafted input. The highest threat from
    this flaw is that it could allow arbitrary code to be executed on the target system. (CVE-2021-41160)
